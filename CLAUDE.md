...
- **An Epic's lifecycle is its single `status` field, specified in [`prompt-sessions/README.md`](session-manager-operations/prompt-sessions/README.md#lifecycle).** That section is the functional requirement — states (`draft` → typing, `proposed` → agent-filed, `active` → session running and authoring PRDs, `completed` → archived), the one start path both doors converge on, and the known gaps. Don't restate the states anywhere else; link there.
