...
| 2026-07-27-01-chat-final-text-turn-not-surfaced | 🆕 open | Filed from burrow project side — a pure-text (no trailing tool call) final assistant turn didn't render in the chat UI at all; identical content re-emitted next turn rendered fine. No file:line available (filer wasn't in this codebase); flagged as possibly a message-type/flush-timing issue on no-tool-call turns. Needs a repro pass in this repo. |
