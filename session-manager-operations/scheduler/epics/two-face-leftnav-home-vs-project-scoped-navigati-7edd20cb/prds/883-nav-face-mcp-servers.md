---
title: "Nav face: MCP Servers both-face, default scope flips user<->project"
...
Make the MCP Servers tab (`src/renderer/components/tabs/McpServers.tsx` ~lines 21-22, 61-113,
dual-scope: `'user'` -> `~/.claude.json`, `'project'` -> `<cwd>/.mcp.json`) default its existing
...
- [ ] Confirm/add `faces: ['home','project']` for the `mcp` key in
      `src/renderer/lib/navGroups.ts` NAV_ITEMS
- [ ] McpServers.tsx computes navFace via `deriveNavFace` and, only on an actual face transition
...
- [ ] timeout 120 npx vitest run <the new/updated McpServers test file> passes
...
this PRD independently. Note: `src/renderer/components/tabs/__tests__/
McpServers.description.test.ts` currently has uncommitted local changes per git status — read it
before adding new tests to avoid duplicating or clobbering that work.

# Out of scope

- Changing the ScopeSwitcher UI component itself or the `.mcp.json`/`~/.claude.json` read/write
  format
