---
title: "Fix: drain stranded legacy PRD dir + verify already-shipped 638 is not re-fired"
...
Job `638-doc-experience-backend` failed on 2026-07-31 with exit `-1` in ~200 ms, before the model
was ever invoked. **Do NOT re-implement PRD 638 — its work already shipped.** This PRD drains the
stranded legacy PRD directory that caused the spurious re-fire, and adds a regression test so the
stranding cannot recur.
...
```
[scheduler] starting 638-doc-experience-backend at 2026-07-31T14:04:48.442Z
[scheduler] cwd=/home/bilko/Projects/session-manager

[scheduler] failed to read PRD: ENOENT: no such file or directory, open
'/home/bilko/Projects/session-manager/session-manager-operations/scheduler/prds/638-doc-experience-backend.md'
```

No `claude -p` child was ever spawned. The failure is entirely in `executeJob`'s PRD-read step
(`src/main/scheduler.cjs:1629-1659`).

## The work already shipped

PRD 638 ran successfully on **2026-07-24** — run dir
`~/.claude/session-manager/scheduled-plans/runs/2026-07-24T20-38-57-256Z/`, whose
`638-doc-experience-backend.verdicts.json` reads `"verdict": "clean"`, `"sentinel": "pass"`.
Both deliverables are present in HEAD today:

- `src/main/docEdit.cjs` exists.
- `files:duplicate` is registered at `src/main/files.cjs:380`, with its pure name generator at
  `src/main/files.cjs:243` and its zod schema at `src/main/ipcSchemas.cjs:439`.

So the 2026-07-31 runs (`10-58-23-768Z`, `14-04-48-116Z`, `14-04-55-632Z`, `14-04-57-126Z` — four
re-fires, three of them within nine seconds) are spurious resurrections of an already-terminal
slug, not real work.

## Why the re-fire could not read its PRD

The PRD source file still lives in the **legacy global dir**:
`~/.claude/session-manager/scheduled-plans/prds/638-doc-experience-backend.md`. Its frontmatter is
`cwd: ~/Projects/session-manager` — tilde-prefixed, not absolute. It is one of **224** `.md` files
still in that dir, **222** of which have a `cwd: ~/...` frontmatter.

PRD 808 moved PRD sources to `<cwd>/session-manager-operations/scheduler/prds/`
(`src/main/lib/prdLocations.cjs`), with a boot migration
(`src/main/lib/prdMigration.cjs` → `runPrdMigration()` in `src/main/scheduler.cjs:560`) that moves
legacy files into their project dir based on the frontmatter `cwd`.

**The app that is actually running is not this git checkout.** It is the npx install at
`/home/bilko/.npm/_npx/5346543b21849140/node_modules/claude-code-session-manager` (verify with
`ps aux | grep electron`). That installed build is behind HEAD in three load-bearing ways:

1. **`prdMigration.cjs` has no tilde expansion.** The installed copy does
   `const cwd = fm.cwd && fm.cwd.trim(); if (!fs.existsSync(cwd)) { unresolved.push(...) }` —
   `existsSync('~/Projects/session-manager')` is always false, so **every** `cwd: ~/...` PRD is
   classified `cwd does not exist on disk` and left in the legacy dir at every single boot. That is
   why 222 files are stranded. Local HEAD fixed this in commit `e87a618`
   ("expand tilde-prefixed cwd in PRD migration existsSync check") — see
   `src/main/lib/prdMigration.cjs:64`, `const cwd = expandHome(rawCwd);`.
2. **`executeJob` has no legacy-dir fallback.** `grep -c "PRD not in project dir"` on the installed
   `src/main/scheduler.cjs` returns `0`. HEAD's commit `b4bfaf5` added a `findPrdDir(job.slug)`
   fallback (`src/main/scheduler.cjs:1638-1650`) that searches the legacy dir before failing the
   job. Without it, the per-project ENOENT is immediately fatal.
3. **No terminal-slug guard and no completion archive.** HEAD has `c05587b` ("stop resurrecting/
   re-investigating already-terminal PRD slugs") and `7f34a6d` ("auto-archive a PRD's .md when its
   job completes"). Either would have prevented the 2026-07-31 re-fire outright.

## Root cause, one sentence

Not a defect in this repo's HEAD: the fixes all exist in git, but the running npx build predates
them, so the tilde-cwd legacy PRDs were never migrated, the completed 638 slug was never archived,
and its resurrection ENOENT'd with no fallback.

# Concrete fix steps

Do these in order. **Do not** edit `src/main/docEdit.cjs` or `src/main/files.cjs` — PRD 638's
deliverables are already correct and shipped.

### Step 1 — Confirm the premise before changing anything

```bash
test -f src/main/docEdit.cjs && echo "docEdit present"
grep -n "files:duplicate" src/main/files.cjs src/main/ipcSchemas.cjs
grep -n "expandHome" src/main/lib/prdMigration.cjs
grep -n "PRD not in project dir" src/main/scheduler.cjs
```

All four must produce output. If any is empty, **HALT** and `exit 1` with a diagnostic line —
the assumptions in this PRD no longer hold and the fix below would be wrong.

### Step 2 — Drain the stranded legacy PRD dir (data repair, one-shot)

Write a small throwaway script (prefer a temp `.cjs` file over a fragile `-e` one-liner) that
requires this repo's own `src/main/lib/prdMigration.cjs` and runs the **fixed** migration against
the legacy dir — reusing the tested code path rather than hand-rolling a mover:

```bash
cat > /tmp/drain-prds.cjs <<'EOF'
const { migratePrds } = require('/home/bilko/Projects/session-manager/src/main/lib/prdMigration.cjs');
const dir = require('node:path').join(require('node:os').homedir(), '.claude/session-manager/scheduled-plans/prds');
migratePrds(dir).then((r) => {
  console.log(`moved=${r.moved} skipped=${r.skipped} unresolved=${r.unresolved.length}`);
  for (const u of r.unresolved) console.log(`  LEFT ${u.file}: ${u.reason}`);
}).catch((e) => { console.error(`HALT: ${e.message}`); process.exit(1); });
EOF
timeout 120 node /tmp/drain-prds.cjs
```

Expect `moved` to be in the low hundreds. Any file reported under `LEFT` is one a human must fix
(genuinely missing/bad frontmatter `cwd`) — list them in your finish output, do not delete them.

Note this moves each PRD into its own project's dir, which for most of them is **not** this repo.
That is correct and intended (PRD 808's model); only the session-manager-owned ones land under
`/home/bilko/Projects/session-manager/session-manager-operations/scheduler/prds/`.

### Step 3 — Archive the already-completed 638 slug

After step 2, `638-doc-experience-backend.md` will have landed in this repo's per-project PRD dir.
Since its work shipped on 2026-07-24, move it out so the scheduler cannot fire it again:

```bash
cd /home/bilko/Projects/session-manager
D=session-manager-operations/scheduler
mkdir -p "$D/prds-archived"
if [ -f "$D/prds/638-doc-experience-backend.md" ]; then
  git mv "$D/prds/638-doc-experience-backend.md" "$D/prds-archived/" 2>/dev/null \
    || mv "$D/prds/638-doc-experience-backend.md" "$D/prds-archived/"
  echo "archived 638"
else
  echo "638 already absent from prds/ — nothing to archive"
fi
```

Do **not** hand-edit `~/.claude/session-manager/scheduled-plans/queue.json` — the Electron app owns
it and is running; a concurrent write would corrupt it. If the queue entry for
`638-doc-experience-backend` is still `running`/stuck after this PRD lands, that is for a human to
clear via the app's Scheduler tab or the `scheduler_reset_job` admin tool.

### Step 4 — Regression test: tilde-cwd PRDs must migrate

`src/main/__tests__/prdMigration.test.cjs` already exists. Add a case there (do not create a new
test file) asserting that a legacy PRD whose frontmatter is `cwd: ~/<something-under-a-temp-HOME>`
is **moved**, not left `unresolved`. Set a temp `process.env.HOME` for the case so it does not
depend on the real home dir, and restore it afterwards. This is the test that would have caught the
installed build's missing `expandHome`.

### Step 5 — Rebuild so the running install can be refreshed

The stale npx install is the ultimate cause, but republishing to npm is **out of scope for this
headless run** (it is a human-owned release action). Just verify the local build is green:
...
In your finish output, state plainly that the running app is the stale npx install at
`/home/bilko/.npm/_npx/5346543b21849140/...` and that a human must republish/relaunch to pick up
`e87a618`, `b4bfaf5`, `c05587b`, and `7f34a6d`. Do not attempt `npm publish` yourself.

# Verification commands

Run these, in this order, with the AC gate last:

```bash
cd /home/bilko/Projects/session-manager
timeout 300 npm run typecheck
timeout 300 npx vitest run src/main/__tests__/prdMigration.test.cjs
timeout 300 npm run test:unit
```

Plus the drain check — note the inversion so the clean case exits 0:

```bash
LEGACY="$HOME/.claude/session-manager/scheduled-plans/prds"
N=$(ls "$LEGACY"/*.md 2>/dev/null | wc -l)
echo "legacy PRDs remaining: $N"
if [ -f "$LEGACY/638-doc-experience-backend.md" ]; then
  echo "HALT: 638 still stranded in the legacy dir"; exit 1
fi
echo "clean"
```
...
- [ ] Verified and stated in the finish output that PRD 638's deliverables already exist
      (`src/main/docEdit.cjs`, `files:duplicate` in `src/main/files.cjs` + `src/main/ipcSchemas.cjs`)
      and that **no re-implementation was performed**.
- [ ] The legacy dir `~/.claude/session-manager/scheduled-plans/prds/` has been drained via
      `migratePrds` from this repo's HEAD; the moved/unresolved counts are reported, and every
      still-`unresolved` file is listed by name and reason.
- [ ] `~/.claude/session-manager/scheduled-plans/prds/638-doc-experience-backend.md` no longer
      exists, and the file now sits in
      `session-manager-operations/scheduler/prds-archived/638-doc-experience-backend.md` in this
      repo.
- [ ] `src/main/__tests__/prdMigration.test.cjs` gains a case proving a `cwd: ~/...` legacy PRD is
      migrated rather than left `unresolved`; the case fails if `expandHome` is removed from
      `src/main/lib/prdMigration.cjs`.
- [ ] `npm run typecheck`, `npm run lint:selectors`, and `npm run test:unit` are all green.
- [ ] `queue.json` was not hand-edited, and no `npm publish` was attempted.
- [ ] The finish output names the stale npx install path and the four commits it is missing, as a
      human follow-up.
