---
title: Markdown preview selection popover — quick-action Claude edits with inline diff
...
The core of the "Document Experience" design: select text in the Editor's markdown preview →
a popover offers quick actions (Make concise · To bullet list · Fix the tone) and a free-text
command box → the PRD-638 `docEdit.run` rewrites the span → a review card shows old vs new →
Accept applies the change to the buffer (autosave persists it), Reject discards, Retry re-runs.
Typed commands only; voice entry and the Assistant rail are PRD 640.

# Acceptance criteria

- [ ] New hook `src/renderer/components/tabs/editor/useDocEdit.ts` — a small phase machine
      `idle | popover | thinking | diff` holding `{ selection, instruction, diff }`, exposing
      `select/cancel/run/retry/accept`. `run` calls `window.api.docEdit.run({ path, before,
      instruction })`; `{ ok:false }` (incl. `busy` and `timeout`) → `toast.error` + back to
      `popover`. `retry` re-runs with the same instruction plus a vary hint suffix.
- [ ] Selection capture in preview mode: on mouseup inside the `.markdown-body` page,
      a non-collapsed `window.getSelection()` whose text is 3..8000 chars enters `popover`
      with the selection text + viewport-anchored rect; clicks elsewhere or Escape cancel.
      Implemented inside `MarkdownPreview` via a wrapper (prop-driven callback), keeping
      MarkdownPreview itself dumb: EditorView owns the hook and passes handlers down.
- [ ] Popover UI (`SelectionPopover.tsx` under tabs/editor/): three quick-action chips —
      Make concise / To bullet list / Fix the tone — each mapping to a fixed instruction
      string, plus a single-line text input ("tell Claude what to change…", Enter runs) and a
      dismiss ×. Positioned above the selection rect, clamped to the pane.
- [ ] `thinking` state renders the popover with a pulsing "Claude is editing…" row and a
      Cancel; Cancel abandons the result (ignore the resolved promise).
- [ ] `diff` state renders a review card anchored like the popover: old text
      (red/strikethrough), new text (green), buttons Accept / Reject / Retry. Styling stays in
      the repo's dark Tailwind idiom (`bg-bg-elev`, `border-line`; red-400/green-* accents).
- [ ] Accept applies via a pure helper `applySpanEdit(buffer, before, after)` in
      `src/renderer/lib/applySpanEdit.ts`: replaces **exactly one** occurrence of `before`;
      if the span occurs zero times (buffer changed under us) or 2+ times (ambiguous), it
      returns `{ ok:false, reason }` and the UI toasts + leaves the diff open. On `{ ok:true }`
      EditorView calls `setBuffer(path, next)` — dirty tracking + autosave then behave exactly
      as a manual edit. Markdown source vs rendered text caveat: `before` is the *rendered*
      selection, so run the match against the buffer first and if not found fall back to a
      whitespace-normalized match (collapse runs of whitespace) mapped back to buffer offsets;
      if even that fails → `{ ok:false, reason:'not-found' }`.
- [ ] The whole feature mounts only for markdown files in `preview` or `split` mode; edit,
      wysiwyg, html, table, jsonl panes are untouched.
- [ ] TDD, red first: `src/renderer/lib/__tests__/applySpanEdit.test.ts` — unique match,
      zero matches, multiple matches, whitespace-normalized fallback (newline in buffer vs
      space in selection), and the mapped-back offsets replacing the right region.
- [ ] `timeout 300 npm run typecheck` passes.
- [ ] `timeout 300 npm run test:unit` passes.

# Implementation notes

Depends on **PRD 638** (`window.api.docEdit.run`, `DocEditResult` in `src/preload/api.ts`) —
verify present or `exit 1`. Independent of the 639 toolbar sibling
(639-doc-toolbar-document-menu): both touch EditorView.tsx in different regions (header vs
body); keep your edits scoped to the body/preview wiring to minimize merge friction, and
rebase/re-read the file if the sibling landed first.

Read first: `src/renderer/components/tabs/EditorView.tsx` (body dispatch :366-415, preview
branches :383-400), `src/renderer/components/tabs/editor/MarkdownPreview.tsx` (page canvas,
forwardRef scroll container), `src/renderer/state/editor.ts` (`setBuffer`, buffers),
`src/renderer/state/toast.ts`.

Quick-action instruction strings (keep as a const map): concise → "Rewrite this text to be
concise and specific. Preserve meaning and any markdown structure."; bullets → "Convert this
text into a tight markdown bullet list."; tone → "Fix the tone: direct, professional, no
hedging. Preserve meaning.".

Design source: Claude Design "Document Experience.html" → `SelPopover` (quick chips + dismiss),
`Sentence` diff rendering (strike-through before / highlighted after / Accept-Reject-Retry).
The mock selects per-sentence spans; the real implementation uses native text selection — do
not pre-split the document into sentence spans.

# Out of scope

- Voice capture, transcript review state, and the Assistant rail (PRD 640).
- Multi-span or whole-document edits; one contiguous selection at a time.
- Persisting edit history; Accept is just a buffer edit.
