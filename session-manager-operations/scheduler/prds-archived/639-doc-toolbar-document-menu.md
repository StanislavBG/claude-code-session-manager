---
title: Editor grouped toolbar — Document actions menu + Display popover + wide measure
...
Reorganize the Editor header (src/renderer/components/tabs/EditorView.tsx:306-360) into the
grouped toolbar from the "Document Experience" design: a **Document ▾** menu owning everything
you can do to the file (Rename, Duplicate, Delete, Copy as Markdown, Add to Claude context,
Open in default app, Reveal), and a **Display** popover replacing today's inline strip of tiny
pref buttons, gaining a "Wide measure" toggle for the markdown preview page. Pure renderer
work over existing + PRD-638 IPC.

# Acceptance criteria

- [ ] New `src/renderer/components/tabs/editor/DocumentMenu.tsx`: button `Document ▾` in the
      EditorView header; popover menu grouped Edit / Open / Export with a fixed-inset backdrop
      that closes on outside click (copy the backdrop pattern used by existing popovers, e.g.
      the kebab menus in SchedulePanel — search for `fixed inset-0` under src/renderer).
      Items and wiring:
      - **Rename** — inline modal (reuse the visual shell of `CloseConfirm` in
        EditorView.tsx:440) with a text input prefilled with the basename; calls
        `window.api.files.rename`; on success updates the open-file entry via the editor
        store and keeps the buffer (see `state/editor.ts` for the openFiles shape — add a
        `renameOpenFile(oldPath, newPath)` action there).
      - **Duplicate** — `window.api.files.duplicate(path)` (PRD 638), then opens the copy as a
        new tab (`useEditor` open action) and toasts the new name.
      - **Delete document** — danger-styled, confirm modal, `window.api.files.delete(path)`,
        then closes the tab through the existing `requestClose` flow bypassing the dirty guard
        (file is gone) and toasts.
      - **Copy as Markdown** — `navigator.clipboard.writeText(buffers[path] ?? '')`, toast
        "Copied N chars".
      - **Add to Claude context** — writes `@<path> ` (trailing space, no newline — must NOT
        auto-submit) to the active terminal tab's pty via `window.api.pty.write({ tabId,
        data })`, using the active tab id from the same source EditorView/Files already use
        (`useActiveTab`); disabled with a tooltip when no terminal tab is active.
      - **Open in default app** / **Reveal** — move the two existing header buttons
        (EditorView.tsx:357-358) into the menu unchanged.
- [ ] Every menu action surfaces failure via `toast.error(...)` — no silent catch.
- [ ] New `src/renderer/components/tabs/editor/DisplayPopover.tsx`: replaces the inline pref
      strip (EditorView.tsx:329-337) with a popover of labeled rows — Text size (A−/current/A+),
      Word wrap, Minimap, Theme (Dark/Paper), Autosave, and new **Wide measure** — all backed
      by the existing `useEditorPrefs` store plus a new persisted `wideMeasure: boolean`
      (default false) following how the store persists its other prefs.
- [ ] `MarkdownPreview` page canvas honors wide measure: the page's max-width goes from its
      current value to ~55% wider when `wideMeasure` is on (pass a prop from EditorView; do
      not read the store inside MarkdownPreview — it stays a dumb renderer).
- [ ] Header layout after the change, left→right: name+path+saved label · Outline toggle ·
      view-mode toggle · spacer · Document ▾ · Display · Focus. No behavior regressions:
      Cmd/Ctrl-S, focus mode, autosave, view modes untouched.
- [ ] TDD: `src/renderer/lib/__tests__` gains a test for the store rename action
      (`renameOpenFile` remaps openFiles/buffers/dirty/viewMode keys, preserves dirty state)
...
Depends on **PRD 638** for `window.api.files.duplicate` — verify it exists in
`src/preload/index.cjs` before starting; if absent, `exit 1` citing the missing dependency.

Read first: `src/renderer/components/tabs/EditorView.tsx` (459 lines, header block :306-360),
`src/renderer/state/editor.ts` (store actions + `OpenFile`), `src/renderer/state/editorPrefs.ts`,
`src/renderer/components/tabs/editor/MarkdownPreview.tsx` (page canvas), `src/renderer/state/toast.ts`,
`src/renderer/lib/useActiveTab.ts`, and `window.api.pty.write` in `src/preload/index.cjs:36`.

Styling: keep the repo's Tailwind idiom (`text-[10px]`/`text-xs`, `border-line`, `bg-bg-elev`,
`text-fg-faint`). This is the dark session-manager shell — do NOT port the design mock's paper
palette or its fonts; port the *structure* (grouping, menu, popover) only. Do not import
Almanac/Hive primitives.

Design source: Claude Design "Document Experience.html" → `DOC_ACTIONS` menu (Edit/Open/Export
groups + danger Delete) and `PrefsPopover` (Text size / Wide measure / Paper tint). Paper tint
maps to the existing Dark/Paper theme pref — no new tint pref.

# Out of scope

- Version history and Export as PDF menu items (cut; leave no dead menu entries).
- The selection popover / edit-diff flow (PRD 639 sibling: 639-doc-selection-edit-diff) and
  voice/Assistant rail (PRD 640).
- Any main-process change.
- Open in VS Code as a distinct item — "Open in default app" covers it.
