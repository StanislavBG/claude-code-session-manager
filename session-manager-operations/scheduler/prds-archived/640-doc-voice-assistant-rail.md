---
title: Doc experience — voice entry for selection edits + Assistant rail
...
Complete the "Document Experience" design: a mic affordance on the selection popover captures
a spoken command (existing Whisper pipeline), shows the transcript for confirmation ("Heard:
… — Send / Re-record / Cancel"), then feeds the PRD-639 edit flow; and a collapsible
**Assistant rail** on the right of the Editor mirrors the whole lifecycle (idle hint,
listening, heard, editing…, proposed diff with its own Accept/Reject/Retry).

# Acceptance criteria

- [ ] `useDocEdit` (from PRD 639-doc-selection-edit-diff) gains phases `listening` and
      `review` between `popover` and `thinking`, with `listen()`, transcript state, and
      `sendHeard()`.
- [ ] The selection popover gains a primary **Voice edit** button. Clicking it starts a
      recording session using the existing voice pipeline — read
      `src/renderer/state/voice.ts` and reuse its capture path scoped so the transcript is
      routed to the doc-edit hook and NEVER to `pty.write` (the store has exactly one
      pty-submitting path, voice.ts:540 — do not add another; if the store cannot cleanly
      target a non-terminal consumer, capture via the same recorder primitives
      (`VoiceButton`/`LiveTranscript` components or the underlying recorder utils) rather
      than forking the store).
- [ ] **Privacy invariant honored**: `RecordingStatus` mounts whenever recording is active —
      verify the App-level mount covers this path; if it keys off the voice store's
      `isRecording`, reusing the store satisfies it; if not, fix the wiring so it does.
- [ ] `listening` UI: pulsing waveform/dots + "Listening…" + cancel, in both the popover and
      the rail. `review` UI: italic "«transcript»" + Send / Re-record / Cancel.
- [ ] New `src/renderer/components/tabs/editor/AssistantRail.tsx` (~300px, right side of the
      preview area, markdown preview/split only): header "Assistant" + model label from
      `lib/prettyModel.ts` (docEdit pins sonnet — display that, do not hardcode a marketing
      name); body mirrors `useDocEdit` phase: idle → "Select text in the doc…" hint + count of
      edits this session; listening/review/thinking as above; diff → before/after blocks with
      Accept/Reject/Retry driving the same hook actions as the inline card.
- [ ] Rail toggle button in the Editor header next to the Outline toggle; collapsed state
      persisted in `useEditorPrefs` (`assistantRail: boolean`, default true); hidden entirely
      in focus mode and for non-markdown files.
- [ ] Edits-this-session counter lives in the hook (increments on Accept), resets per file.
- [ ] TDD, red first: unit test for the extended phase machine (transitions
      popover→listening→review→thinking→diff→accept, cancel from every phase returns to
      idle, re-record loops review→listening) — extract the machine's transition logic into a
      pure reducer if needed to make it testable without DOM/audio.
- [ ] `timeout 300 npm run typecheck` passes.
- [ ] `timeout 300 npm run test:unit` passes. Do NOT run the e2e suite.

# Implementation notes

Depends on **PRD 639-doc-selection-edit-diff** (hook + popover + diff). Verify
`useDocEdit.ts` exists or `exit 1`.

Read first: `src/renderer/state/voice.ts` (975 lines — the invariant comments around :77 and
:540 are load-bearing), `src/renderer/components/VoiceButton.tsx`,
`src/renderer/components/LiveTranscript.tsx`, `src/renderer/components/RecordingStatus.tsx`
and its mount in `src/renderer/App.tsx`, `src/renderer/lib/prettyModel.ts`,
`src/renderer/state/editorPrefs.ts`.

The rail is a pane in the modular-pane sense: it owns no global state, receives the hook via
props from EditorView, and renders read-mostly mirrors of it. Dark Tailwind idiom; no Almanac/
Hive primitive imports; port the design's structure, not its paper palette.

Design source: Claude Design "Document Experience.html" → `SelPopover` (listening/review
states), `AssistantRail` (idle/listening/review/thinking/diff cards), toolbar Assistant
toggle.

# Out of scope

- Auto-submit of the transcript without user confirmation — Send is always explicit.
- TTS / spoken responses; any change to terminal voice command routing.
- Streaming partial transcripts into the instruction box (final transcript only).
- The Document menu / Display popover (639 sibling) — different header region.
