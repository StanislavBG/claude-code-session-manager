---
title: Fix 4 red chatRunner.spec.ts tests left by silent-run queue-leak fix (db171bd)
...
`npm run test:unit` is red on main: 4 tests in `tests/unit/chatRunner.spec.ts` fail. They
assert the pre-db171bd silent-run concurrency semantics (cap=2 queues the 3rd probe; manual
runs uncapped; per-tab manual-vs-silent guard; recordExchange gating), and commit db171bd
("fix silent-run queue leak and multi-select capture") changed `src/main/chatRunner.cjs`
behavior without updating them. Reconcile the tests with the intended post-db171bid behavior —
or, if db171bd genuinely broke the concurrency contract, fix chatRunner and keep the tests.

# Acceptance criteria

- [ ] First, decide which side is wrong: read `git show db171bd -- src/main/chatRunner.cjs`
      and the failing assertions (`tests/unit/chatRunner.spec.ts:230,277,335` and the
      recordExchange-gating test). The CLAUDE.md invariant that MUST survive either way: no
      more than 3 concurrent `claude -p` processes machine-wide; chatRunner's own silent-probe
      lane cap (SM_CHAT_CONCURRENCY clamp) still enforced. If db171bd lost the cap (3 silent
      probes running when 2 expected — the first failure suggests exactly that), fix
      `chatRunner.cjs`; if the cap moved intentionally, update the tests' expectations and
      their comments to describe the new contract.
- [ ] All 16 tests in `timeout 120 npx vitest run tests/unit/chatRunner.spec.ts` pass.
- [ ] `timeout 420 npm run test:unit` fully green (812+ tests, 0 failed).
- [ ] `timeout 300 npm run typecheck` passes.
- [ ] Commit message states which side was wrong and why.

# Implementation notes

Failures as of ae0e43b:
- `queues a 3rd concurrent silent probe behind the 2 already running` — expected 2 spawned,
  got 3 (cap not enforced or default cap changed).
- `a manual run does not inflate activeCount / starve a silent probe` — expected 2, got 0
  (silent probes not spawning at all in this scenario).
- `a manual run for a tab already running a silent probe is dropped` — silentChild null.
- `skips recordExchange and turn-affecting broadcasts when silent: true`.
The mix (one over-spawn, two under-spawn) suggests db171bd's queue-leak fix changed WHEN jobs
are admitted (e.g. drained queue on completion only) — reproduce before fixing, per the
debugging standard. Verified pre-existing at 69c1d7a (before the doc-experience PRDs), so do
not suspect docEdit.cjs/files.cjs — they are unrelated.

# Out of scope

- Any behavior change to manual (non-silent) chat runs beyond what the chosen contract needs.
- docEdit.cjs (has its own single-flight latch; not part of this lane).
- e2e tests.
