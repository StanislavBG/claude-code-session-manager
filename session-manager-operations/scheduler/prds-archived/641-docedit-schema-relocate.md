---
title: Move docEditRun zod schema into ipcSchemas.cjs (single-source IPC validation)
...
PRD 638 landed the `docedit:run` payload schema as a local `docEditRun` z.object inside
`src/main/docEdit.cjs:100` instead of the repo's single home for IPC schemas,
`src/main/ipcSchemas.cjs`. Relocate it so every IPC boundary schema lives in one file, per
the codebase convention (see ipcSchemas.cjs header and how memoryTool/files register via
`validated(schemas.X, handler)`).

# Acceptance criteria

- [ ] `docEditRun` schema moved verbatim into `src/main/ipcSchemas.cjs` (same constraints:
      path ≤4096, before 1..8000, instruction 1..2000, `.strict()`), exported via the
      `schemas` object.
- [ ] `src/main/docEdit.cjs` registers via the shared pattern:
      `ipcMain.handle('docedit:run', v(s.docEditRun, handler))` using
      `require('./ipcSchemas.cjs')`'s `{ schemas, validated }` — matching
      `registerMemoryHandlers` in `src/main/memoryTool.cjs:195-202`. No local z.object or zod
      import remains in docEdit.cjs.
- [ ] Behavior identical: invalid payloads still rejected; `src/main/__tests__/docEdit.test.cjs`
      still passes unchanged (pure helpers untouched).
- [ ] `timeout 300 npm run typecheck` passes.
- [ ] `timeout 120 npx vitest run src/main/__tests__/docEdit.test.cjs tests/unit/ipc-memory.spec.ts` passes.

# Implementation notes

Read `src/main/docEdit.cjs` (140 lines) and the schema block layout in
`src/main/ipcSchemas.cjs:320-364`. Place `docEditRun` near the files* schemas with a one-line
comment naming its consumer. This is a pure move — resist any refactor beyond it.

# Out of scope

- Any behavior/validation change; renderer; other schemas.
