---
title: Make chatRunner.spec.ts hermetic against SM_CHAT_CONCURRENCY in the dev shell
...
`tests/unit/chatRunner.spec.ts` fails 4 tests on any machine whose shell exports
`SM_CHAT_CONCURRENCY` (the user's ~/.bashrc sets 3): the spec's "concurrency (default
cap = 2)" block asserts default-cap behavior, but `src/main/chatRunner.cjs:239` reads
`process.env.SM_CHAT_CONCURRENCY` when the cap is computed, so the developer's shell leaks
into the assertions. Confirmed live 2026-07-24: with the var set to 3 → 4 failures
("expected 2 got 3" etc.); `env -u SM_CHAT_CONCURRENCY` → 16/16 pass. Make the spec
deterministic regardless of inherited env.

# Acceptance criteria

- [ ] The spec pins the env before the cap is computed — `delete process.env.SM_CHAT_CONCURRENCY`
      (or `vi.stubEnv`) in the spec's setup, positioned so it takes effect before
      `chatRunner.cjs` computes `CONCURRENCY_CAP`. Check WHEN the cap is read
      (module-load vs per-call at src/main/chatRunner.cjs:231-239): if it is captured at
      import time, the spec must reset modules (`vi.resetModules()` + dynamic import after
      stubbing) or chatRunner must read the env lazily — prefer the smallest change that
      keeps production behavior identical.
- [ ] Add one test asserting the override still works: with SM_CHAT_CONCURRENCY stubbed to 1,
      a 2nd silent probe queues behind the 1st (proves the clamp path stays covered).
- [ ] `SM_CHAT_CONCURRENCY=3 timeout 120 npx vitest run tests/unit/chatRunner.spec.ts` passes
      (this is the regression gate — run it with the var explicitly set).
- [ ] `env -u SM_CHAT_CONCURRENCY timeout 120 npx vitest run tests/unit/chatRunner.spec.ts` also passes.
- [ ] `timeout 300 npm run typecheck` and `timeout 420 npm run test:unit` pass.

# Implementation notes

Read `src/main/chatRunner.cjs:225-245` (cap computation + clamp) and the spec's existing
setup/mocks. Context: session-manager-operations/reviews/chatrunner-concurrency-reconciliation.md
records the 2026-07-24 investigation — an executor (clean env) saw 16/16 green while the
interactive shell (var=3) saw 4 red; this PRD closes that gap. If you touch chatRunner.cjs at
all, behavior with the env var set must remain byte-identical in production (clamp [1,3],
default 2).

# Out of scope

- Changing the cap default, clamp range, or any chatRunner runtime behavior.
- Auditing other specs for env sensitivity (separate sweep if wanted).
