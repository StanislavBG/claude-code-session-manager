---
title: Auto-archive completed PRD .md files after DoD coverage
...
`~/.claude/session-manager/scheduled-plans/prds/` holds 738 .md files and only grows — every
NN allocation full-dir scan (prdParser.cjs:181-228), every reconcile listing, and every
lint-queue pass walks all of them though ~100% belong to long-completed jobs. Auto-archive a
completed job's PRD file into the existing `prds-archived/<ISO-ts>/` layout once it is safe:
job terminal, covered by a definition-of-done report (or past the same 7-day retention used
by PRD 644), and not the target of any pending/running fix-plan.

# Acceptance criteria

- [ ] New function in src/main/queueOps.cjs (reuse its existing `archiveMany` — do NOT write
      a second mover): `autoArchiveCompleted(state, { nowMs })` selects slugs where the job is
      `completed`, `finishedAt` older than the shared retention constant (import from
      lib/schedulerConfig.cjs, added by PRD 644 — if absent, define HISTORY_RETENTION_MS
      there yourself with default 7 days), no `NN-fix-<slug>` sibling is pending/running, and
      the slug is not itself a pending/running job. `needs_review` and `failed` PRD files are
      NEVER auto-archived (humans and the auto-fix loop still need them in place).
- [ ] Wired into the scheduler's reconcile-adjacent maintenance (same cadence as PRD 644's
      partition — after it, so a job archives from jobs[] first and its file next pass or
      same pass). Reconcile already tolerates a vanished .md (it drops the queue entry), but
      with 644's split the terminal job is in history.jsonl by then — assert order so a hot
      completed job's file is never removed before its queue row is archived.
- [ ] Archived moves are logged one line each (slug → dest) and are atomic `fsp.rename` via
      archiveMany's existing path-containment. Never deletes; prds-archived only.
- [ ] Kill-switch `SM_PRD_AUTOARCHIVE_DISABLE=1` skips entirely.
- [ ] NN allocation stays monotonic after archiving: allocateParallelGroup
      (prdParser.cjs:181-228) scans the live prds/ dir, so archiving old `NN-*.md` files
      would let NN regress and collide with archived groups. Fix by having
      allocateParallelGroup ALSO consider a high-water mark: persist `maxAllocatedGroup` (in
      queue.json config or a sidecar the allocator owns) updated on every allocation and on
      every scan that sees a higher NN; allocation result = max(scan, high-water) + 1. Test
      covers: archive all files, allocator still returns a number above the historical max.
- [ ] TDD, red first: unit tests for the selection predicate (fresh completed stays,
      old completed archives, needs_review never, fix-plan-target never, pending never) and
      the high-water NN behavior.
- [ ] `timeout 300 npm run typecheck` and `timeout 420 npm run test:unit` pass.

# Implementation notes

Read first: src/main/queueOps.cjs:212-244 (archiveMany), src/main/lib/prdParser.cjs:181-228
(allocateParallelGroup + .reserved-NN markers — the high-water mark must count reservation
markers too), scheduler.cjs reconcile (:599), and PRD 644's queueHistory.cjs if landed.

Depends on PRD 644 conceptually but must be safe standalone: if 644 has not landed, the
selection predicate still works against jobs[] statuses; the ordering assertion just becomes
"job row still present and completed".

# Out of scope

- Archiving needs_review/failed/pending PRDs; any UI; retention for prds-archived itself.
- Changing the NN-prefix parallel-group convention.
