---
title: Coalesce schedule:state broadcasts (debounce full-payload pushes)
...
`broadcast()` (src/main/scheduler.cjs:785-791) sends the full buildScheduleStatePayload over
`schedule:state` on every state mutation — a burst (batch launch, reconcile refreshing many
rows, reaper pass) emits N consecutive full payloads and the renderer re-renders each one.
Coalesce broadcasts with a trailing-edge debounce so a mutation burst produces one push,
while a lone mutation still arrives promptly.

# Acceptance criteria

- [ ] `broadcast()` debounced ~200 ms trailing-edge (constant `BROADCAST_COALESCE_MS` in
      lib/schedulerConfig.cjs): first call arms a timer; further calls within the window
      reset nothing (trailing edge fires once with the LATEST state); timer `unref()`d.
- [ ] A forced immediate flush path for moments where latency matters: pause/resume
      transitions and job start/finish call `broadcast({ flush: true })` which cancels the
      pending timer and sends now (the renderer's pause-toast logic in scheduleState.ts:57-70
      depends on seeing pause transitions).
- [ ] IPC `schedule:state` invoke (pull) is untouched — always returns current state.
- [ ] No broadcast is ever dropped: after the debounce window, the payload reflects all
      mutations in the window (payload is built at send time, not enqueue time).
- [ ] TDD, red first: unit test with fake timers — 5 rapid broadcasts → 1 send with final
      state; flush bypasses the window; a send after quiet period arrives.
- [ ] `timeout 300 npm run typecheck` and `timeout 420 npm run test:unit` pass.

# Implementation notes

Read src/main/scheduler.cjs:757-791 (buildScheduleStatePayload + broadcast + sendIfAlive) and
every `broadcast(` call site (grep) to pick which get `flush: true` — err on the side of
flushing state-machine transitions (paused, resumed, job started/finished) and coalescing the
rest (reconcile refreshes, progress-ish updates). Renderer needs no change: scheduleState.ts
just receives fewer, newer snapshots.

# Out of scope

- Payload diffing/partial updates; renderer changes; webRemote (pull-based, unaffected).
