---
title: Surface SM_SCHEDULER_MAX_CONCURRENCY env override in the Scheduler UI
...
When `SM_SCHEDULER_MAX_CONCURRENCY` is set, the effective cap is `ENV_CAP ?? config.concurrencyCap`
(src/main/scheduler.cjs:227-229, :1849) — the PolicyBar's concurrencyCap number input
(SchedulePanel.tsx:290) then silently does nothing, which reads as a bug. Surface the
override: payload carries the effective cap + its source, and the UI disables the input with
an explanatory tooltip when env-pinned.

# Acceptance criteria

- [ ] `buildScheduleStatePayload` (scheduler.cjs:757) gains
      `effectiveConcurrency: { cap: number, source: 'env' | 'config' }` computed from
      ENV_CAP ?? state.config.concurrencyCap. Type added wherever the snapshot payload type
      lives in src/preload/api.ts / renderer types.
- [ ] PolicyBar (src/renderer/components/SchedulePanel.tsx:266-334): when source === 'env',
      the concurrencyCap input is disabled, shows the effective value, and a title tooltip
      says: pinned by SM_SCHEDULER_MAX_CONCURRENCY — unset the env var to edit. A small
      `env` badge (existing badge idiom in sched-primitives — but do NOT add to
      sched-primitives itself; inline in PolicyBar) sits next to the label.
- [ ] When source === 'config', behavior is exactly today's (editable, setConfig works).
- [ ] TDD, red first: a unit test for the payload field (env set → source 'env' + env value
      wins; unset → 'config'); if scheduler.cjs's ENV_CAP is read at module load, structure
      the test accordingly (lazy read or module reset) — mirror how existing scheduler unit
      tests handle env (see src/main/__tests__/ for the pattern), and keep production
      behavior identical.
- [ ] `timeout 300 npm run typecheck` and `timeout 420 npm run test:unit` pass.

# Implementation notes

Read scheduler.cjs:227-229 (ENV_CAP clamp [1,20]), :1849 (enforcement), :757 (payload),
SchedulePanel.tsx:266-334 (PolicyBar). Note the env-sensitivity lesson: the test must pass
both with and without SM_SCHEDULER_MAX_CONCURRENCY exported in the invoking shell — stub/
delete it explicitly in setup rather than inheriting.

# Out of scope

- Changing precedence (env keeps winning); other env overrides; webRemote.
