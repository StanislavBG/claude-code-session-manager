---
title: Chat — render the answer body on needs-input turns, not just the question card
...
When a Chat run ends with the stop-signal question protocol, the renderer shows ONLY the
"needs your answer" card and silently drops everything the agent wrote before the sentinel —
so "describe the five steps" yields a question card and no five steps (live incident,
screenshot evidence 2026-07-25). The full text already reaches the renderer as `raw` on the
`chat:run:needs-input` event; fix the pipeline so the answer body renders as a normal
assistant bubble with the question card beneath it, gets recorded to durable history, and the
prompt instruction steers the model to answer-then-ask.

# Acceptance criteria

- [ ] New pure export `splitStopSignal(finalText)` in `src/main/chatRunner.cjs` beside
      `parseStopSignal` (:71-77): returns `{ answerBody, questions }` where `answerBody` is
      everything before the LAST `STOP_SENTINEL` occurrence, with the trailing sentinel line
      and the JSON `{"questions":[...]}` line removed and whitespace trimmed. `parseStopSignal`
      stays (other callers); implement it as a thin wrapper over the new split or vice versa —
      one parser, not two (single source of truth).
- [ ] The `chat:run:needs-input` payload (:512-518) gains `answerBody` (keep `raw` for
      back-compat one release; the renderer stops using it).
- [ ] `src/renderer/state/chat.ts:230-236` (`onNeedsInput`): when `answerBody` is non-empty,
      push TWO turns — a normal assistant turn `{ text: answerBody, toolUses: <the run's
      accumulated tool uses> }` followed by the question turn `{ text: questions.join('\n'),
      questions }`. Empty `answerBody` → question turn only (today's shape). Tool-use trace
      attaches to the answer turn, not the question card.
- [ ] `TerminalChat.tsx` question-card rendering (:288) unchanged in look; the answer bubble
      renders through the existing assistant branch (markdown, UrlCallout, ToolUseTraceStrip).
- [ ] Durable history: needs-input turns now also call `recordExchange` (:520-523 currently
      skips them) with `result: answerBody + '\n\n[asked: ' + questions.join(' | ') + ']'` —
      fire-and-forget off the hot path exactly like the complete branch.
- [ ] `STOP_SIGNAL_INSTRUCTION` (:222-227) amended: before the sentinel rule, add one
      sentence — first deliver everything you CAN answer or produce now (findings, the
      requested content, work completed) as normal output; ONLY THEN, if something still
      blocks you, end with the sentinel + questions. Never make the question your entire
      reply when the user asked for content.
- [ ] TDD, red first: extend the chatRunner spec (or a new
      `tests/unit/chatStopSignal.spec.ts`) — split with body+question; sentinel-only (empty
      body); body containing an EARLIER literal sentinel string (lastIndexOf semantics);
      multi-question JSON; malformed JSON after sentinel → treated as complete (existing
      parseStopSignal null path preserved). Renderer state test: needs-input with answerBody
      → two turns in order; without → one.
- [ ] `timeout 300 npm run typecheck` and `timeout 420 npm run test:unit` pass (delete
      SM_CHAT_CONCURRENCY from env in any new spec setup — the suite must pass with and
      without it exported).

# Implementation notes

Read first: src/main/chatRunner.cjs:54-77 (sentinel + parseStopSignal), :222-227
(STOP_SIGNAL_INSTRUCTION), :498-527 (result handling — note `text` prefers event.result over
accumulated finalAssistantText), src/renderer/state/chat.ts:17-40 (turn shape: `questions?`
field doc at :33) and :225-240 (event subscriptions), TerminalChat.tsx:280-330 (question card
+ assistant branch), src/preload/index.cjs:410-415 (onNeedsInput pass-through — add answerBody
to the typed payload in src/preload/api.ts).

The evidence screenshot for this incident: /tmp/session-manager-clipboard/clipboard-1784948256308.png
(three consecutive question-only cards where the user asked for content each time).

# Out of scope

- The empty-assistant-bubble render guard (PRD 649, separate feedback item).
- Multi-turn question threading / answering questions inline in the card (composer reply
  stays the answer path).
- Any change to the silent-probe path (silent runs never parse stop signals today — keep).
