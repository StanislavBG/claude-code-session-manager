---
title: Chat — never render an empty assistant bubble (thinking-only / empty-text turns)
...
TerminalChat renders the assistant turn verbatim with no guard for empty text — a resumed
turn that opens with a thinking block (not rendered) paints a content-less "C" bubble that is
indistinguishable from a hang (feedback item
session-manager-operations/feedback/2026-07-21-chat-empty-assistant-bubble.md, screenshot in
its evidence/ folder). Add the guard: running-with-no-text shows a working affordance;
finished-with-no-text shows an explicit placeholder or is suppressed.

# Acceptance criteria

- [ ] In `src/renderer/components/TerminalChat.tsx` assistant branch (:309-329): when
      `turn.text` is empty/whitespace AND the run is still active (derive from the chat
      store's run status for this tab — read src/renderer/state/chat.ts for the
      status field), render the existing ToolUseTraceStrip plus a pulsing "working…"
      affordance instead of the empty bubble (reuse an existing pulse/dots idiom from the
      codebase — grep for animate-pulse — do not invent a new spinner).
- [ ] When a turn is FINAL with empty text but has toolUses: render the trace strip plus a
      muted italic placeholder "(no textual reply — see tool activity above)".
- [ ] When a turn is final, empty, and has NO toolUses: suppress the bubble entirely.
- [ ] Non-empty turns render exactly as today (markdown, UrlCallout, plan styling).
- [ ] TDD, red first: extract the branch decision into a pure helper
      `assistantTurnPresentation(turn, runActive)` in src/renderer/lib (returns
      'text' | 'working' | 'placeholder' | 'suppress') with a unit test covering all four
      cases + whitespace-only text; TerminalChat switches on it.
- [ ] After landing, move the feedback file to the inbox's processed/archived location per
      session-manager-operations/feedback/README.md convention (read it first; if no README
      convention exists, leave the file and note that in the commit message).
- [ ] `timeout 300 npm run typecheck` and `timeout 420 npm run test:unit` pass.

# Implementation notes

Depends on PRD 648 landing first (both edit TerminalChat.tsx; 648 restructures the
needs-input path — build on its state of the file). Read the feedback file's Evidence section
for the exact transcript timing of the thinking-then-text sequence this guards.

# Out of scope

- Streaming partial text into the bubble (separate feature); chatRunner changes; the
  background-shell false-promise feedback item (separate root cause, separate fix).
