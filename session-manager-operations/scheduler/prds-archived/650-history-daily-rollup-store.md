---
title: History — durable daily rollup store; closed days served precomputed
...
The History dashboard recomputes everything from raw transcripts (~21k .jsonl / 1.7 GB live)
behind a process-local LRU — every Electron restart cold-starts the scan, PARSE_BUDGET_MS=2s
silently truncates cold results, and history is capped by Claude Code's transcript retention
because session-manager keeps no copy. Introduce a durable daily rollup at
`~/.claude/session-manager/history-rollup.jsonl`: one JSON line per (localDate, projectDir,
modelId) with the scalar aggregates. Closed (pre-today) days are served from the rollup —
instant, never truncated, and they survive transcript cleanup. Today stays live-parsed.

# Acceptance criteria

- [ ] New module `src/main/lib/historyRollup.cjs` exporting pure helpers + thin IO:
      `rollupKey(date, projectDir, modelId)`, `mergeRollupLines(lines)` (last-write-wins per
      key), `readRollup(fromDate, toDate)` (returns Map key→bucket), and
      `appendRollupDays(entries)` (append-only via config.cjs atomic/append helpers; dedupe
      on read, compact on write when the file exceeds 5 MB by rewriting merged lines).
      Bucket fields: promptCount, inputTokens, outputTokens, cacheReadTokens,
      cacheCreationTokens, toolCallCount, toolBreakdown, errorCount, sessionCount. Prices are
      NOT stored — cost stays computed at read time from MODEL_PRICING so a pricing fix
      retroactively corrects displayed cost (single source of truth,
      historyAggregator.cjs:23-27).
- [ ] `historyAggregator.cjs` aggregate path split: for dates < today (local), consult the
      rollup first; only files whose sessionDate is missing from the rollup are parsed and
      then folded INTO the rollup (self-healing backfill). Today's files always live-parse
      exactly as now (LRU + tail-parse untouched).
- [ ] A rollup day is only trusted as complete when its `finalizedAt` stamp exists; the
      finalize pass (exported `finalizeClosedDays()`) scans transcripts for any day older
      than today not yet finalized, writes its buckets + finalizedAt. Wired to run once at
      app boot (deferred ~30 s, fire-and-forget, logged) — the cron path is PRD 651.
- [ ] Deleted-transcript immunity: aggregate results for a finalized day must come from the
      rollup even when the underlying .jsonl files no longer exist on disk.
- [ ] `history:aggregate` response gains `{ rollupDays, liveDays }` counts so the dashboard
      can show provenance later; existing response fields unchanged (HistoryDashboard needs
      no edit in this PRD).
- [ ] PARSE_BUDGET_MS truncation can now only affect today's slice: verify by test that a
      budget-exhausted scan with a fully-finalized range still returns complete closed-day
      rows with truncated=false for those days.
- [ ] TDD, red first: `src/main/__tests__/historyRollup.test.cjs` — merge last-write-wins,
      date-range read, compact-on-threshold, finalize skips today, deleted-file immunity
      (finalize from fixture transcripts, delete them, aggregate still returns the day),
      backfill fold-in.
- [ ] `timeout 300 npm run typecheck` and `timeout 420 npm run test:unit` pass.

# Implementation notes

Read first: src/main/historyAggregator.cjs in full (494 lines) — especially parseJSONL
cache/tail logic (:210-300), the aggregate walk + PARSE_BUDGET_MS (:311-431), localDate
helper, decodeCwd (:78), and MODEL_PRICING/resolvePricingKey (:23-44). Reuse localDate,
decodeCwd, resolvePricingKey — do not fork. File IO through config.cjs helpers (writeJson /
writeTextAtomic / append if present — check what exists; never hand-roll tmp+rename).

Timezone: sessionDate is derived via localDate (America/Los_Angeles machine-local) — the
rollup keys on the same localDate so a day closes at local midnight; use the same helper
everywhere.

Sessions count: a transcript file = one session; sessionCount per bucket = number of files
whose sessionDate lands that day (count once per file per day, not per model — put
sessionCount on a per-(date,project) line with modelId '' to avoid double-counting).

# Out of scope

- Cron/offline refresh (PRD 651). Dashboard UI changes. Retention/pruning of the rollup
  itself. Changing MODEL_PRICING values or moving pricing into the rollup.
