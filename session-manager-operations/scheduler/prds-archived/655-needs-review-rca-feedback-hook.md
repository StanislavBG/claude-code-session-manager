---
title: Scheduler — file a Root Cause Analysis feedback item on every needs_review verdict
...
When the verifier parks a job in `needs_review`, the diagnosis dies with the run: the auto-fix
investigation (when it fires) spends Opus tokens reading the log/transcript and pours it all
into a fix PRD; when it doesn't fire, nothing is recorded at all. Add
`src/main/lib/rcaFeedbackHook.cjs` — fired on the needs_review transition, mirroring the
dodDrainHook pattern (fire-and-forget, idempotent, kill-switched) — that writes a Root Cause
Analysis markdown into the target project's feedback inbox so /process-feedback can fold the
lesson back into that project's PRD authoring.
...
Read first: src/main/lib/dodDrainHook.cjs (the pattern: guards, idempotency, fire-and-forget),
scheduler.cjs verdict mapping (:1645-1665), spawnInvestigation + its completion handling
(:1464+), runs/ meta.json shape (exit, durationMs, rateLimited), and one existing feedback
file for the markdown idiom (e.g. session-manager-operations/feedback/2026-07-21-chat-empty-assistant-bubble.md
in this repo — frontmatter + What happens + Evidence + Suggested direction).

Rate-limit pauses are NOT needs_review and must never file RCAs. `uncommitted_changes` DOES
file one (it is the least self-healing verdict). Keep the lib electron-free (no
require('electron')) so tests stay hermetic and the watchdog could reuse it later.

# Out of scope

- Any LLM call in the deterministic path; changing auto-fix eligibility/caps; a UI surface
  for RCAs (they are feedback files; /process-feedback owns consumption); retention/pruning
  of filed RCAs; changes to the external watchdog.
