---
title: History rollup v2 — active minutes, schema re-finalize, intraday today-line, scan-free dashboard IPC
...
Prepare the rollup store to be the History tab's ONLY data source (PRD 660 builds the new
dashboard on it): (1) add an `activeMinutes` metric per bucket (sum over transcripts of
lastEventTs - firstEventTs per day), (2) version the rollup schema and re-finalize days
written under v1 so minutes backfill automatically, (3) keep a provisional "today" line
refreshed in-app every 5 minutes so the renderer NEVER triggers a transcript scan, and
(4) expose `history:dashboard` — an IPC that answers entirely from rollup lines: current +
prior window slices for ranges 30/60/90/all with per-project/per-model/per-day buckets.

# Acceptance criteria

- [ ] `src/main/lib/historyRollup.cjs`: bucket gains `activeMinutes` (number, default 0 on
      old lines at read time); rollup lines gain `v: 2`. `isDateFinalized` treats a day
      finalized under v1 (no `v` or v<2) as NOT finalized, so the existing
      finalizeClosedDays pass re-scans and upgrades it — bounded by the existing budgetMs
      resume mechanism; no new migration machinery.
- [ ] `historyAggregator.cjs` per-file parse also captures the LAST event timestamp (it
      already captures the first for sessionDate); per-file activeMinutes =
      clamp((lastTs-firstTs)/60000, 0, 24*60). Cached LRU entries lacking lastTs re-parse on
      next touch (bump the aggregator's internal cache-entry shape guard rather than
      clearing the cache wholesale if a shape check already exists — read the entry shape at
      :67-74 first).
- [ ] Intraday refresher: in the main process, every `HISTORY_INTRADAY_REFRESH_MS`
      (5 min, schedulerConfig.cjs) — and immediately at app start after the boot finalize —
      recompute TODAY's buckets via the existing live-parse path (LRU-warm, cheap) and
      upsert them as provisional lines (`v:2`, no finalizedAt). Timer unref'd; skipped when
      a finalize lock is held. `readRollup` last-write-wins already dedupes on key — verify
      provisional lines for the same day replace older provisional lines on compact.
- [ ] New IPC `history:dashboard` (zod schema in ipcSchemas.cjs, preload method + type):
      request `{ rangeDays: 30|60|90|0 }` (0 = all time); response
      `{ from, to, days: [{date, byProject: {proj: {bucket incl cost fields computed}}}],
      prevTotals, totals, byProjectTotals, byModelTotals, toolsByProject, generatedAt,
      provisionalDates: [today?] }` — cost computed at read time from MODEL_PRICING via
      resolvePricingKey (reuse; do not store prices). The handler reads rollup lines ONLY —
      assert in a test that it performs zero transcript-file reads (inject a spy/fs guard or
      verify by module boundaries: it may only import historyRollup + pricing helpers).
- [ ] Existing `history:aggregate` stays untouched (SessionLog and any other consumers keep
      working) — the new dashboard just stops calling it.
- [ ] TDD, red first: minutes clamp math; v1-line read defaults; isDateFinalized v-gating
      (v1 day gets re-finalized); provisional upsert/compact replacement; dashboard response
      shape for a 3-day fixture rollup incl. prior-window totals and all-time (rangeDays 0);
      zero-scan guarantee test.
- [ ] `timeout 300 npm run typecheck` and `timeout 420 npm run test:unit` pass (clean env).

# Implementation notes

Read first: src/main/lib/historyRollup.cjs in full (landed in commit 5eb4db2 — exports
rollupKey/mergeRollupLines/readRollup/isDateFinalized/appendRollupDays/compact, plus
TOTALS_MODEL_ID/FINALIZED_* sentinel ids — understand the sentinel-line scheme before
extending), historyAggregator.cjs finalize + live-parse paths, watchdogHelpers
maybeFinalizeHistory (stamp/lock), schedulerConfig.cjs. Live rollup on disk:
~/.claude/session-manager/history-rollup.jsonl (real v1 lines to test read-compat against —
copy a few into a fixture, do NOT read the live file in tests).

The watchdog needs no change: its daily finalize picks up v2 automatically via the
isDateFinalized gate.

# Out of scope

- Any renderer change (PRD 660). Removing history:aggregate. Prices in the rollup.
  Per-hour buckets. Changing the watchdog cadence.
