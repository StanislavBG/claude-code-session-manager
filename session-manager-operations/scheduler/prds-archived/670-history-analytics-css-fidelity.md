---
title: History analytics — apply the design's full typographic + CSS fidelity layer
...
The rewritten History analytics dashboard (commit 58706b5) landed structurally complete on
the correct paper palette, but was built from a spec revision that predated the CSS-fidelity
correction: it lacks the design's typographic hierarchy and exact styling values. Apply the
"Visual language — CORRECTION" block verbatim — it already exists, fully specified, in
`~/.claude/session-manager/scheduled-plans/prds/660-fix-history-analytics-dashboard-ui.md`
(section starting "Visual language — CORRECTION"); read it FIRST, it is the authoritative
spec for this PRD. This is styling only: no data, IPC, or view-model changes.

# Acceptance criteria

- [ ] Page h1 and every SectionHead title render in `font-serif` (Newsreader) at the design's
      scale (~42px h1 with an italic accent em; ~25px section titles, negative tracking);
      kickers ("01 · over time" style) are uppercase mono ~10.5px in accent.
- [ ] All numerals mono: headline value ~52px mono negative tracking, substats ~17px mono,
      ranking values, panel figures.
- [ ] Cards use 14px radius on the card-token background with 1px edge border (the design's
      cardBase), consistently across headline, trend, ranking, drill, and panels.
- [ ] Control bar is sticky with translucent paper background (`rgba(246,239,225,0.93)`) +
      `backdrop-blur` + bottom edge border.
- [ ] Delta component uses #566b34 (good) / #a3441f (bad), inverted for Spend; stacked-trend
      dimmed bands use #ded2bd. Define these once (extend tailwind.config.js or one shared
      constants module) — no scattered hex literals.
- [ ] Segmented controls, facet chips, share bars, CachePanel donut (r=30/stroke 9/sage),
      Rhythm bars (#cfc0a2, accent peak) match the design's dimensions per the CORRECTION
      block's component list.
- [ ] No functional regressions: existing analytics unit tests pass unchanged
      (`timeout 120 npx vitest run src/renderer/lib/__tests__/analyticsFormat.test.ts
      src/renderer/lib/__tests__/historyFacet.test.ts src/renderer/lib/__tests__/usageCsv.test.ts`).
- [ ] `timeout 300 npm run typecheck` and `timeout 420 npm run test:unit` pass.

# Implementation notes

Read first: the CORRECTION block in 660-fix-history-analytics-dashboard-ui.md (authoritative);
then src/renderer/components/tabs/history/analytics/ (all files, landed in 58706b5),
tailwind.config.js (token map — bg/paper #f6efe1, accent #b85c34, serif Newsreader already
configured). Verify the Newsreader/Geist/IBM Plex Mono fonts are actually bundled/loaded by
the app (grep index.html / styles.css for @font-face or font imports) — if a face is missing,
add it via the app's existing font-loading mechanism, never a runtime Google Fonts fetch
(CSP + offline-first).

You ARE the executor: implement these styles directly in this run. Do not invoke /develop or
any queue-authoring skill.

# Out of scope

- Any data/ETL/IPC change; new panels; light/dark theming work; SessionLog.
