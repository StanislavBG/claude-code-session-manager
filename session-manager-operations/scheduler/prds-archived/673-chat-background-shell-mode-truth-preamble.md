---
title: Tell the Chat-mode agent that background shells don't survive between turns
...
The Chat tab (`TerminalChat`) runs each turn as a one-shot headless `claude -p` job via
`src/main/chatRunner.cjs` — its own header literally says "Chat · headless session — no
process runs between commands" (`src/renderer/components/TerminalChat.tsx:488`). Nothing in
the prompt the agent actually receives says this, so an agent can (and did) launch a
`run_in_background: true` Bash shell to poll something long-running, promise "I'll report
once it lands," then end its turn — after which the shell is torn down at process exit and
no wake-up ever fires. The user is left with ~18 minutes of silence until they poke the
session manually (feedback item
`session-manager-operations/feedback/2026-07-21-chat-background-shell-false-promise.md`,
full transcript evidence with line numbers/timestamps inside). Fix: tell the agent the truth
about the mode in the prompt it receives, the same way `chatRunner.cjs` already injects
`STOP_SIGNAL_INSTRUCTION` into every prompt.

# Acceptance criteria

- [ ] In `src/main/chatRunner.cjs`, add a new prepended instruction constant (sibling to
      `STOP_SIGNAL_INSTRUCTION` at line 241) — e.g. `CHAT_MODE_TRUTH_INSTRUCTION` — stating
      plainly: this is a one-shot headless run; no process survives after this turn ends;
      `run_in_background` shells, scheduled wake-ups, or "I'll get back to you when X
      finishes" plans are impossible here because nothing will resume the session — any
      polling must be done synchronously within this turn with a bounded timeout, and the
      turn must end with either a real result or an explicit statement that the user needs
      to reply to continue.
- [ ] Prepend it to `fullPrompt` alongside `STOP_SIGNAL_INSTRUCTION` at
      `src/main/chatRunner.cjs:413` (order doesn't matter functionally; keep both distinct
      constants — don't merge them into one string, they cover different concerns and should
      stay independently editable).
- [ ] TDD, red first: add/extend a test in `tests/unit/chatRunner.spec.ts` asserting the
      spawned `claude` process's `-p` argv value contains the new instruction text (mirror
      however the existing spec already asserts on `STOP_SIGNAL_INSTRUCTION`/`fullPrompt`
      content, if such an assertion exists — grep the spec file first; if none exists yet,
      add one for both constants so this doesn't regress silently).
- [ ] `timeout 300 npm run typecheck` and `timeout 300 npx vitest run tests/unit/chatRunner.spec.ts` pass.
- [ ] After landing, move
      `session-manager-operations/feedback/2026-07-21-chat-background-shell-false-promise.md`
...
- Read `src/main/chatRunner.cjs:239-250` for the exact style/format `STOP_SIGNAL_INSTRUCTION`
  uses (a template string ending in `\n\n`, prepended at line 413:
  `const fullPrompt = STOP_SIGNAL_INSTRUCTION + prompt;`). Follow the same pattern — plain
  prose, no markdown headers, imperative and specific.
- This is a prompt-injection fix, not a UI fix — no renderer changes needed. Guard 2 from the
  feedback item (a visible "⏳ background poll won't resume" banner in the composer) is
  explicitly OUT of scope for this PRD — it's a separate, larger renderer change with its own
  design tradeoffs; only ship the prompt-truth fix here.
- Don't touch the `STOP_SIGNAL_INSTRUCTION` constant itself or its stop-sentinel protocol —
  this is an additive, independent instruction.

# Out of scope

- The composer/thread "fail loud" visible banner (feedback item's guard 2) — a separate,
  larger UI feature; not required for this fix to land.
- Any change to `run_in_background` Bash-tool behavior itself (that's harness-level, not this
  app's).
- The related "empty assistant bubble" rendering bug from the same incident — already covered
  by PRD `649-chat-empty-assistant-bubble-guard`; do not duplicate it here.
