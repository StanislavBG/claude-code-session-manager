---
title: Commit-guard falls back to committedInWindow so branch-hopping PRDs don't false-flag pass_no_commit
...
The scheduler's live commit-guard (`src/main/scheduler.cjs:1702-1733`) detects whether a job
committed work by comparing `gitHead(guardCwd)` before vs. after the run on `job.cwd`'s
*current* branch only. A PRD that legitimately checks out one or more *other* branches,
commits real work on each, pushes, then checks its starting branch back out before exit (a
documented shared-repo hygiene pattern) ends the run with `HEAD === guardHeadBefore`, so
`committedDuringRun` comes out `false` even though real commits landed elsewhere. `runVerify.cjs:725`
then raises `pass_no_commit` and downgrades a fully-correct, truthfully-`PASS`-sentineled run
to `needs_review`. Confirmed live: PRD `599-sharp-osv-fix-batch-b` hit exactly this (commits
`5f94c94`, `9b2d054`, `a560c88`, `d7b147e` on four non-starting branches, pushed to `fork`,
flagged anyway). Fix: when the HEAD-diff check says "no commit," fall back to the existing
`committedInWindow()` helper (already used on the re-verify path,
`src/main/scheduler.cjs:2539`), which scans `git log --all --since=<start> --until=<finish>`
across every ref — not just the starting branch's HEAD — before concluding no commit happened.

# Acceptance criteria

- [ ] In `src/main/scheduler.cjs`, at the live commit-guard (around line 1732-1733, inside the
      `if (res.exitCode === 0 && !res.rateLimited)` block), when
      `committedDuringRun` computed from `gitHead` before/after is `false`, call
      `await committedInWindow(guardCwd, job.startedAt, new Date().toISOString())` (the run has
      just finished at this point, so "now" is a valid upper bound for the window — `job.finishedAt`
      is not yet set on the in-memory `job` object here) and OR its result into
      `committedDuringRun` before passing it to `verifyRun({ ..., committedDuringRun })`.
- [ ] `committedInWindow` (defined `src/main/scheduler.cjs:210`) is reused as-is — do not fork
      or duplicate its `git log --all --since/--until` logic; import/call the existing function.
- [ ] TDD, red first: add a test to `tests/unit/schedulerFinish.spec.ts` (or the file that
      currently covers the live commit-guard / `pass_no_commit` path — grep for
      `committedDuringRun`/`guardHeadBefore` first) that reproduces the branch-hopping scenario:
      a fixture repo where the job's `cwd` starts and ends on the same branch/SHA, but a commit
      lands on a *different* branch within the run's time window — assert `committedDuringRun`
      ends up `true` and the run is NOT downgraded to `pass_no_commit`.
- [ ] Add a regression case for the inverse: no commit anywhere in the window (on any branch)
      still correctly yields `committedDuringRun === false` and `pass_no_commit` still fires —
      this fix must not blanket-suppress the real detector.
- [ ] `timeout 300 npm run typecheck` and `timeout 300 npx vitest run tests/unit/schedulerFinish.spec.ts` pass.
- [ ] After landing, move
      `session-manager-operations/feedback/2026-07-22-pass-no-commit-false-negative-on-branch-hopping-prd.md`
      to `session-manager-operations/feedback/processed/` with a `## Resolution` section naming
      the commit.

# Implementation notes

- Read `src/main/scheduler.cjs:192-215` for `gitHead()` and `committedInWindow()`'s existing
  signatures/behavior before writing the fallback call — `committedInWindow` already handles
  git-unavailable / no-repo cases by returning `false`, never throwing (per its own comment at
  line 208).
- The re-verify path at `src/main/scheduler.cjs:2539` already calls
  `committedInWindow(job.cwd, job.startedAt, job.finishedAt)` — that's the reference call shape;
  the only difference here is `finishedAt` isn't set on `job` yet at the live-guard call site, so
  use `new Date().toISOString()` as the end of the window instead.
- Keep the `gitHead` before/after check as the fast path (cheap, no `git log`) and only pay for
  `committedInWindow`'s `git log --all` scan when the fast path found nothing — don't run both
  unconditionally on every job.

# Out of scope

- Changing `runVerify.cjs`'s `pass_no_commit` raising condition itself — this fix corrects the
  input (`committedDuringRun`) it's fed, not the verifier's logic.
- The `-merge-main` postcondition exemption (`pass_no_commit_target_verified`) — unrelated
  mechanism, already shipped (PRD `575-verifier-merge-postcondition-exemption`).
- Re-scanning/retagging already-completed historical jobs that were mis-flagged before this
  fix — out of scope; the existing `queueOps.cjs` archive/retag tooling (documented
  `PRD_AUTHORING.md` §15) already covers manual recovery if needed.
