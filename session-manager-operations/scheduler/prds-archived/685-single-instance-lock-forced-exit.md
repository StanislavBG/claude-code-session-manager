---
title: Fix single-instance-lock quit path to actually terminate the losing process (not zombie)
...
`src/main/index.cjs:836-854` already implements Electron's single-instance guard correctly in
spirit — `app.requestSingleInstanceLock()` (line 842), and on failure (line 843-844) calls
`app.quit()`. But this was found live, today, to NOT actually terminate the losing process: three
separate `claude-code-session-manager` launches (via `npm exec claude-code-session-manager@latest`)
were found running concurrently for over a day, each with a full live process tree (zygote(s),
`gpu-process`, network utility, `chrome_crashpad_handler`) but **no renderer/window** — i.e. each
one correctly never opened a window (the lock guard worked for that part), but the underlying
Electron/Node process never actually exited and sat there indefinitely as a zombie, still holding
memory and (depending on init order) potentially still running in-process timers.

Root cause: `app.quit()` called this early (synchronously, at module top-level, before
`app.whenReady()` — this call happens before any window exists) only *requests* a graceful quit;
per Electron's own documented behavior, `app.quit()` is not guaranteed to force-terminate the
process, especially when the app never became "ready" or never created a window — there's nothing
for the normal `window-all-closed` → `quit` → process-exit chain to hook into. The fix: force an
immediate, unconditional process exit for the losing instance instead of a soft quit request.

# Acceptance criteria

- [ ] In `src/main/index.cjs`, replace the `app.quit()` call at line 844 (inside
  `if (!gotLock) { app.quit(); }`) with `app.exit(0)` — `app.exit()` (as opposed to `app.quit()`)
  immediately terminates the process without waiting for cleanup handlers or the normal
  quit-event chain, which is exactly what's needed here since this process has done nothing
  besides the lock check (no window, no IPC, no user data at risk).
- [ ] As a defense-in-depth backstop (in case `app.exit()` itself is ever delayed by some Electron
  internal on a future version), add a hard fallback: `setTimeout(() => process.exit(0), 2000)`
  registered immediately alongside the `app.exit(0)` call, so that even if `app.exit()` somehow
  doesn't terminate the process, a raw `process.exit(0)` fires within 2 seconds. Clear/no-op this
  timer is unnecessary — the process should already be gone well before 2s in the normal case.
- [ ] Add a regression test (search `find src/main -iname '*index*spec*' -o -iname
  '*single*instance*'` first for an existing home for main-process startup tests; if none exists,
  it's acceptable to test this via a small integration check under this repo's e2e or a targeted
  unit test that stubs `app.requestSingleInstanceLock`/`app.exit`/`process.exit` and asserts
  `app.exit(0)` is called (not `app.quit()`) on lock failure — follow whatever mocking pattern
  this repo's existing Electron-`app`-stubbing tests use, don't invent a new one).
- [ ] Manually verify (describe the verification, don't literally launch a second GUI instance in
  the headless PRD run — this is a UI/process-lifecycle change untestable headlessly per this
  project's "no interactive AC in PRDs" convention): confirm by code inspection that the new path
  is `requestSingleInstanceLock() fails → app.exit(0) [+ 2s process.exit(0) backstop] → process
  gone`, with no window/IPC/timer ever initialized in that branch (it returns/exits before
  `app.whenReady()` is ever reached, so nothing else in `index.cjs` has run yet — confirm this by
  reading the surrounding code, don't assume).
- [ ] `timeout 300 npm run typecheck` passes.
- [ ] `timeout 120 npx vitest run <any test file you added/touched>` passes.

# Implementation notes

- Read `src/main/index.cjs:836-854` in full before editing — the `isDev`/`SM_E2E` skip (line 840)
  must be preserved exactly as-is; this PRD only changes what happens in the `!gotLock` branch,
  not whether the guard runs at all.
- Do not touch the `second-instance` handler (lines 846-852) — that's the *winning* process's
  logic (bring existing window to front) and is unrelated to this bug, which is entirely about the
  *losing* process failing to actually die.
- `app.exit([exitCode])` vs `app.quit()`: per Electron's docs, `app.exit()` "exits immediately with
  `exitCode`... this method is usually implemented in low-level languages" — it does not wait for
  async operations, does not emit `before-quit`/`will-quit` and is the correct primitive for "this
  process must die right now, it holds nothing that needs graceful cleanup." Do not use it
  anywhere else in the file — only in this specific lock-failure branch.

# Out of scope

- Do not change `bin/cli.cjs` — it has no instance-checking logic today and doesn't need any; the
  fix belongs entirely in `index.cjs` where the lock already lives.
- Do not add any new IPC, UI, or user-facing setting for this — it's a pure process-lifecycle bug
  fix with no visible behavior change for the winning/normal-path instance.
- Do not change dev-mode (`SM_DEV=1`)/e2e (`SM_E2E=1`) behavior — those intentionally skip the
  lock entirely and must continue to do so unchanged.
- Do not attempt to retroactively detect/kill already-zombied instances from a prior bad launch —
  that's an operational cleanup (already done manually for the currently-affected machine), not
  something this PRD's code needs to handle going forward once the fix lands.
