---
title: "Fix: stop flagging already-landed PRD re-runs as pass_no_commit, and guard resetJob on completed jobs"
...
Fix a scheduler false-failure class in **this repo's own code**: when an already-completed PRD is
executed a second time, the (correct) no-op run is flagged `pass_no_commit` → `needs_review` and
burns an Opus auto-fix investigation.

**You ARE the executor. The deliverable of this PRD is a code diff in `src/main/` plus unit tests —
not another PRD.** Do not invoke `/develop`, `/process-feedback`, or any queue-authoring skill.

# Root-cause analysis (read this first — it is the whole context you get)

The failed job was `812-workbench-review-nits-cleanup`. Its work was **already done correctly**; the
failure is in the scheduler, not in that PRD's subject matter. Do **not** re-do any Workbench/dockview
work — commit `00d891c` already landed all of it.

Timeline (all times PDT, 2026-07-31):

| Time | Event |
|---|---|
| 23:56 (prev day) | Run `2026-07-31T06-56-37-186Z` of slug `812-workbench-review-nits-cleanup` starts |
| 00:02:26 | That run commits `00d891c` ("fix(workbench): clean up deferred minor/nit findings…") |
| 00:02:34 | `runVerify` writes verdict **`clean`** → job status `completed` |
| ~00:24 | **The identical slug fires again** (run `2026-07-31T07-24-27-794Z`) |
| 00:25:52 | Second run re-checks every AC, finds all of it already implemented by `00d891c`, runs `npm run typecheck` (clean) and `npx vitest run src/renderer/state/__tests__/layout.test.ts` (17/17 green), declines to make a no-op commit, prints a truthful `SCHEDULER_VERDICT: PASS` |
| 00:25:53 | `runVerify` → **`pass_no_commit`**, `downgradeTo: needs_review`; auto-fix investigation spawned |

Two independent defects produced that outcome.

## Defect 1 — `pass_no_commit` has no "prior run of this slug already landed the commit" exemption

`src/main/runVerify.cjs` (search for `const isFixPlanJob = /^\d+-fix-/`, around line 744) flags
`sentinel === 'pass' && !committedDuringRun` unless one of exactly two exemptions applies:

- `^\d+-fix-` fix-plan slugs (investigations legitimately find nothing to change), and
- `-merge-main` slugs (independently re-checked via `gh`, yielding `pass_no_commit_target_verified`).

An **ordinary PRD re-executed after its own earlier run already committed** matches neither. The
verifier only ever looks at the current run's window, so a correct, honest, idempotent re-run is
indistinguishable from a lying run that did nothing.

This is systemic, not a one-off: slug `807-the-scheduler-display-should-specifically-account-for-these`
hit the identical pattern the same night (run `2026-07-31T07-22-54-068Z`, result text: *"the acceptance
criteria are all satisfied by the already-landed `b998e6a` … Nothing to change, so no new commit is
needed. SCHEDULER_VERDICT: PASS"*), and HEAD commit `62aa990` is a **manual human cleanup** of exactly
those 807 duplicates.

## Defect 2 — a `completed` job can be reset back to `pending` with no guard

`resetJobFields` (`src/main/scheduler.cjs`, ~line 1240) unconditionally sets `status = 'pending'` and
clears `runId`/`verifierVerdict`. Its public entry point `resetJob(slug)` (~line 3586, exposed over the
loopback admin API and the `scheduler_reset_job` MCP tool, both documented as "reset one **stuck** job")
applies it to **any** job — including one that already completed cleanly and landed a commit. Because
the PRD `.md` was still live in `session-manager-operations/scheduler/prds/` (it was never archived —
there is no copy in `prds-archived/`), the reset row was immediately re-picked by `pickNextBatch` and
re-executed, re-running shipped work and paying for a full `claude -p` run plus an Opus investigation.

`reconcile()` already has a deliberate anti-resurrection guard for slugs that left `jobs[]` into
history — `resetJob` is the remaining hole in that same invariant.

**This is NOT the "delegated instead of executed" failure class.** The failure log shows no `Skill`
and no `ScheduleWakeup` invocation; the executor did real verification work. Do not write the fix as if
it were a self-delegation incident.

# Fix steps

Read each file before editing. Line numbers above are approximate — re-locate by content.

## Step 1 — persist the commit a run landed

In `src/main/scheduler.cjs`, `spawnJob` already computes `guardHeadBefore` (via `gitHead(guardCwd)`
before the run) and `guardHeadAfter` after it, and derives `jobSelfCommitted`.

- When a run finishes with `exitCode === 0` and `guardHeadAfter && guardHeadBefore && guardHeadAfter !== guardHeadBefore`,
  persist `landedCommit = guardHeadAfter` onto the job row in the same `mutate(...)` block that sets
  `status`/`finishedAt`/`exitCode`.
- Make sure the field survives `reconcile()` (which rebuilds rows via `{ ...job, <prd fields> }` — a
  spread, so it carries through) and `queueHistory.appendHistory` (verify the archived record isn't
  field-filtered; if it is, add `landedCommit` to the retained set).
- `resetJobFields` must **not** delete `landedCommit` — the whole point is that it outlives a reset.

## Step 2 — new verifier exemption `pass_no_commit_prior_run_verified`

In `src/main/runVerify.cjs`:

- Extend `verifyRun`'s destructured options with `priorLandedCommit` (default `null`) — the commit SHA
  a previous run of this same slug landed, supplied by the caller.
- Add a helper `isAncestorCommit({ cwd, sha, execImpl })` that shells out to
  `git merge-base --is-ancestor <sha> HEAD` in the job's cwd (bounded — reuse the same exec/timeout
  pattern `checkMergeablePr` uses for `gh`; never spawn an unbounded child). Exit 0 → true; any error
  or non-zero → false (fail safe, exactly like the `gh` path).
- In the `sentinel === 'pass' && !committedDuringRun && !isFixPlanJob` branch, **before** pushing the
  `pass_no_commit` issue and after the existing `-merge-main` check: if `priorLandedCommit` is set and
  `isAncestorCommit` confirms it is reachable from the current `HEAD`, `return conclude(...)` with
  verdict `pass_no_commit_prior_run_verified`, `downgradeTo: null`, a reason naming the SHA (e.g.
  `SCHEDULER_VERDICT: PASS with no commit, but a prior run of this slug landed <sha>, which is an
  ancestor of HEAD — the PRD's work is already in the tree, so this re-run correctly made no change`),
  and extras `{ sentinel, priorLandedCommit }`.
- Keep the existing behaviour untouched when `priorLandedCommit` is null or not an ancestor.

## Step 3 — supply `priorLandedCommit` at both call sites

`verifyRun(` is called twice in `src/main/scheduler.cjs` (the post-run verify in `spawnJob`, ~line 2081,
and the rescan path, ~line 2920). At each, look up the most recent **prior** terminal record for the
same slug and pass its `landedCommit`:

- prefer the live `jobs[]` row's own `landedCommit` (survives a `resetJob`, per Step 1),
- fall back to `queueHistory.historyTerminalBySlug()` (already imported/used by `reconcile`) when the
  row left `jobs[]`.

Never pass the commit the *current* run just made — that is what `committedDuringRun` already covers.

## Step 4 — treat the new verdict as success everywhere `pass_no_commit_target_verified` is

Grep for `pass_no_commit_target_verified` and add `pass_no_commit_prior_run_verified` alongside it:

- `src/main/scheduler.cjs` ~line 2198 (the `effectiveStatus = 'completed'` branch),
- `src/main/scheduler.cjs` ~line 2932 (the rescan's `verdict === 'clean' || …` check),
- `src/main/lib/rcaFeedbackHook.cjs` — ensure the new verdict does **not** map into
  `FAILURE_CLASSES.NO_SENTINEL` (see the `pass_no_commit` mapping at ~line 157 and the label map at
  ~line 59); a verified prior-run no-op must file no RCA feedback and trigger no investigation.
- Also confirm it is **not** added to `RESCANNABLE_VERDICTS` (~line 2766) — it is terminal-clean.

## Step 5 — guard `resetJob` against resurrecting completed work

In `src/main/scheduler.cjs`:

- Change `resetJob(slug)` to `resetJob(slug, opts = {})`. If the target job's `status === 'completed'`
  and `opts.force !== true`, return
  `{ ok: false, error: 'job already completed — resetting it would re-execute shipped work; archive the PRD instead, or pass force:true' }`
  without mutating anything. `pending` / `running` / `failed` / `needs_review` keep today's behaviour.
- Thread the flag through the callers that expose this: the admin route in `src/main/adminServer.cjs`
  (see `src/main/__tests__/scheduler-admin-routes.test.cjs` for the existing route contract) and
  `scripts/scheduler-mcp-server.cjs`'s `scheduler_reset_job` tool — accept an optional `force` boolean
  and surface the refusal message verbatim to the caller. Do not silently swallow the refusal.

# Verification

Run each of these; all must be green. Every command is time-bounded — keep it that way.

```bash
timeout 300 npm run typecheck
timeout 180 npx vitest run src/main/__tests__/runVerify.test.cjs
timeout 180 npx vitest run src/main/__tests__/scheduler-admin-routes.test.cjs
timeout 300 npx vitest run src/main/__tests__
```

Note: `SM_CHAT_CONCURRENCY` set in the shell environment makes `chatRunner.spec` red locally while
green headless — if that specific spec is the only failure, `unset SM_CHAT_CONCURRENCY` and re-run
before treating it as a regression.

# Acceptance criteria

- [ ] `src/main/scheduler.cjs` persists `landedCommit` on a job row when a run's `HEAD` advanced, and
      `resetJobFields` does not clear it.
- [ ] `src/main/runVerify.cjs` accepts `priorLandedCommit` and returns verdict
      `pass_no_commit_prior_run_verified` with `downgradeTo: null` when that SHA is an ancestor of
      `HEAD`; unchanged `pass_no_commit` behaviour otherwise (null SHA, non-ancestor SHA, or git error).
- [ ] Both `verifyRun(` call sites in `scheduler.cjs` pass a prior-run `landedCommit` (jobs[] row first,
      `queueHistory` fallback).
- [ ] `pass_no_commit_prior_run_verified` is treated as completed/clean in `scheduler.cjs` at both
      places `pass_no_commit_target_verified` is, files no RCA feedback via `rcaFeedbackHook.cjs`, and
      is absent from `RESCANNABLE_VERDICTS`.
- [ ] `resetJob` refuses to reset a `completed` job without `force: true`, returning the explicit error
      message; `adminServer.cjs` and `scripts/scheduler-mcp-server.cjs` accept and forward `force` and
      surface the refusal.
- [ ] New unit tests in `src/main/__tests__/runVerify.test.cjs` covering, with a stubbed git exec impl:
      (a) PASS + no commit + ancestor `priorLandedCommit` → `pass_no_commit_prior_run_verified`,
      (b) PASS + no commit + non-ancestor SHA → `pass_no_commit`,
      (c) PASS + no commit + git error → `pass_no_commit` (fail-safe),
      (d) PASS + no commit + no `priorLandedCommit` → `pass_no_commit` (regression guard).
- [ ] New unit test covering `resetJob` refusing a `completed` job and still allowing a `needs_review`
      one (extend `src/main/__tests__/scheduler-admin-routes.test.cjs` or add a sibling spec — follow
      the existing file's harness shape, don't invent a new pattern).
- [ ] All four verification commands above pass.
- [ ] Work is committed before the run ends.

# Out of scope

- Any Workbench / dockview / `layout.ts` / `navGroups.ts` / `workbench.css` change — commit `00d891c`
  already landed all of PRD 812's original acceptance criteria. Do not touch those files.
- Re-running or re-authoring `812-workbench-review-nits-cleanup`.
- Changing `autoArchiveCompleted`'s retention window or archiving PRD files by hand.
- Any change to the `-merge-main` or `^\d+-fix-` exemptions' existing behaviour.
