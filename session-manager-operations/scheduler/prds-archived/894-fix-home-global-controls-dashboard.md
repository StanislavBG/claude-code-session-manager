---
title: "Fix: close Global Controls test-coverage gaps so PRD 894 lands a real commit"
...
PRD `894-home-global-controls-dashboard` exited 0 and printed `SCHEDULER_VERDICT: PASS`,
but the job was flagged. **The code was not broken — the run was a no-op.**

The feature PRD 894 asked for had *already been implemented and committed on `main`* before
that run started:

- `src/renderer/components/tabs/GlobalControlsSection.tsx` — the card grid (Settings,
  Permissions, Hooks, MCP Servers, System Prompt, Skills, all `'user'` scope). Landed in
  commit `8b61462`, bug-fixed in `2cf02e5` (empty CLAUDE.md), refactored in `23469ff`
  (shared skills-walk helper).
- Wired into `src/renderer/components/tabs/SessionManagerConfig.tsx` (import at line 3,
  render at line 50, `navigate` prop passed through).
- Unit spec at `src/renderer/components/tabs/__tests__/GlobalControlsSection.test.tsx`
  (2 tests).

So the executor did the right thing mechanically: it re-verified every acceptance criterion
live (`npm run typecheck` green, `npx vitest run` on the spec green, `npm run lint:selectors`
green, zero `src/main` diff), found nothing to change, committed nothing, and reported PASS.

**Why that still fails the gate:** the scheduler's finish protocol treats
`SCHEDULER_VERDICT: PASS` **plus a commit that landed during the run** as the authoritative
"I passed" signal. A truthful PASS with an empty working tree has no landed commit to clear
it, so the verifier parks the job. This is the *"duplicate / already-satisfied PRD → exit 0
with nothing committed"* failure class.

Explicitly **not** a self-delegation failure: the run invoked no `session-manager-dev:develop`
or `:process-feedback` skill, and never called `ScheduleWakeup`. Do not treat it as one.

**What this fix PRD does:** the feature itself is correct and stays. What is genuinely
incomplete is PRD 894's fourth acceptance criterion — *"asserts each card renders its
count/status from the mocked data"* and the navigate behaviour. The existing spec asserts
the Skills card only in its degenerate empty state (`listDir` is mocked to return
`{ entries: [] }`, so it only ever proves the string `0 skill(s) installed`), and it asserts
`navigate` for exactly one of the six cards. Closing those two gaps is a real, in-scope,
committable deliverable — and it makes this run end with a landed commit so the verifier
auto-clears it.

# Do NOT

- Do **not** rewrite, restructure, or "improve" `GlobalControlsSection.tsx` beyond what the
  acceptance criteria below require. The component is on `main` and working.
- Do **not** add any main-process / IPC route. `git diff --stat -- src/main` must stay empty.
- Do **not** re-queue this work, author another PRD, or call `ScheduleWakeup`. See
  Engineering standards below — you are the executor.

# Fix steps

1. Read the current implementation and spec before touching anything:
   - `src/renderer/components/tabs/GlobalControlsSection.tsx`
   - `src/renderer/components/tabs/__tests__/GlobalControlsSection.test.tsx`
   - `src/renderer/components/tabs/SessionManagerConfig.tsx` (lines 1–60, for the
     `navigate` prop contract)

   In particular, find the skills-walk helper introduced by commit `23469ff` and the exact
   `NavKey` string each card passes to `navigate` — the assertions below must use the real
   keys the component uses, not guessed ones (`git show 23469ff --stat` and
   `git show 8b61462 -- src/renderer/components/tabs/GlobalControlsSection.tsx` are the fast
   way in).

2. **Extend the existing spec file** `src/renderer/components/tabs/__tests__/GlobalControlsSection.test.tsx`
   — do not create a second spec file. Its `installWindowApiMock()` currently stubs
   `config.listDir` as `vi.fn().mockResolvedValue({ ok: true, error: null, entries: [] })`.
   Change that mock so it returns a realistic non-empty skills tree matching whatever shape
   the component's skills walk actually consumes (inspect the helper first — it may recurse
   into per-skill directories looking for `SKILL.md`, in which case the mock must answer
   per-path, the same way `readJson`/`readText` already do in that file).

3. Add a test asserting the **Skills card renders a non-zero installed count** from that
   mock (e.g. `2 skill(s) installed`). This is the case the current suite never exercises.

4. Add a test asserting **every** card routes correctly: iterate the six cards, click each
   button, and assert `navigate` was called with that card's `NavKey`. Assert all six keys,
   not just `mcp`. Keep using the existing `dispatchEvent(new MouseEvent('click', { bubbles: true }))`
   pattern already in the file — don't introduce a new testing library.

5. If — and only if — steps 2–4 surface an actual defect in `GlobalControlsSection.tsx`
   (e.g. the skills count is wrong for a nested tree, or a card passes a `NavKey` that isn't
   in `NAV_ITEMS`), fix that defect in the component too, and say so in the commit message.
   Otherwise leave the component file untouched.

6. Commit. The finish protocol requires a commit that lands **during this run** — that is
   the whole point of this PRD. Suggested message:
   `test(sm-config): cover non-empty skills count + all six navigate keys in Global Controls`

# Verification commands

Run these in order, bounded, with the acceptance gate LAST:

```
timeout 300 npm run typecheck
timeout 120 npx vitest run src/renderer/components/tabs/__tests__/GlobalControlsSection.test.tsx
npm run lint:selectors
git diff --stat -- src/main
```

`git diff --stat -- src/main` must print nothing (renderer-only change). Note that a bare
`git diff --stat` producing no output exits 0 — that is the clean case; do not wrap it in a
bare `grep`.

# Acceptance criteria

- [ ] `src/renderer/components/tabs/__tests__/GlobalControlsSection.test.tsx` mocks
      `config.listDir` with a non-empty skills tree and asserts the Skills card renders a
      non-zero installed count
- [ ] The same spec asserts `navigate` is called with the correct `NavKey` for **all six**
      cards (Settings, Permissions, Hooks, MCP Servers, System Prompt, Skills), not just one
- [ ] No new spec file is created; the existing spec is extended
- [ ] `git diff --stat -- src/main` reports zero changes
- [ ] `timeout 300 npm run typecheck` passes
- [ ] `timeout 120 npx vitest run src/renderer/components/tabs/__tests__/GlobalControlsSection.test.tsx`
      passes (strictly more tests than the 2 it had before)
- [ ] `npm run lint:selectors` passes
- [ ] A commit lands during this run containing the test changes, and the run ends with
      `SCHEDULER_VERDICT: PASS` as its literal last line
