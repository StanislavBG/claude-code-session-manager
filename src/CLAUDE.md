...
- **`main/templates/` provenance.** `PRD_AUTHORING.md` is hand-edited and version-seeded by its
  ...
  never refresh. Any `project-pages-catalog.json` / `project-pages-pipeline.md` are generated by
  `web/project-pages` — never hand-edit; `project-pages-default-home.html` is hand-maintained.
