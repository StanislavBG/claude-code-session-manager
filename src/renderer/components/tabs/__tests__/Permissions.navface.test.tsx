...
/**
 * Permissions's scope switcher defaults from the NavFace
 * (leftnav-two-face-framework): Home face -> 'user', Project face ->
 * 'project' (falling back to 'user' if no active-tab cwd resolves). Mirrors
 * Hooks.navface.test.tsx's auto-default-unless-manually-touched coverage.
 */
...
function installWindowApiMock() {
  const raw = JSON.stringify({ permissions: {} }, null, 2) + '\n'
  const api = {
...
async function mount() {
  ...
  await act(async () => {
    root!.render(createElement(Permissions))
...
describe('Permissions NavFace-driven default scope', () => {
