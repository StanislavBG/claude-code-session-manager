 ...
 * explainer.
 *
 * **All five lenses are tabs here**, `home` included, and this is now the only
 * place any of them is rendered. Home was originally left out because
 * ProjectHome.tsx hosted `home.html` in its own iframe directly above this
 * section — which meant the screen carried two document viewers stacked on top
 * of each other showing the same page, the upper one with no tab strip and no
 * full-screen view, while the generator, `LENS_ORDER` and this section's own
 * explainer all said there were five templates. That upper block is gone; this
 * strip opens on Home so the screen still leads with it.
 *
...
export function ProjectPagesSection({
  ...
  // Home first: it's the screen's lead document now that ProjectHome no longer
  // renders a second copy of it above this section.
