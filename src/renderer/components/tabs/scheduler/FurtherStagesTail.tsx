...
export function FurtherStagesTail({ stages, onSeek }: { stages: Stage[]; onSeek: (i: number) => void }) {
  ...
  return (
    <div data-testid="further-stages" style={{ width: TAIL_W }} className="shrink-0 border-l border-rule-structural flex flex-col">
      <div className="h-[30px] shrink-0 flex items-center px-3 border-b border-rule-inner text-[10.5px] font-semibold uppercase tracking-wide text-fg-faint">
        Stage {from}–{to}
      </div>
      <div className="h-[22px] shrink-0 flex items-center gap-1 px-3 text-[10.5px] font-semibold uppercase tracking-wide text-fg-faint">
        Further stages
