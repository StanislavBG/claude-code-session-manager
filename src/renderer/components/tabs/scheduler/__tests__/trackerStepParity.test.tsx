...
describe('tracker step parity', () => {
  it('completed 6-step DAG + independent follow-up → 2 plans, 7 steps', () => {
    ...
    expect(plans[0].doneCount).toBe(6)
    expect(plans[0].status).toBe('done')
    expect(plans[1].prdCount).toBe(1)
    expect(plans[1].stages[0].n).toBe(1)
    expect(plans[1].stages[0].rows[0].status).toBe('pending')
...
describe('plan band header', () => {
  ...
  it('states step total and done count including archived steps', () => {
    ...
    root = createRoot(el)
    act(() => root!.render(
      <PlanBand plan={plans[0]} now={0} hidden={new Set()} indexBySlug={new Map()} headChoicesBySlug={new Map()} onRowFocused={() => {}} />,
    ))
