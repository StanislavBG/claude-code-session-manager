...
export const CATALOG_HOOKS: CatalogHook[] = [
  ...
  { id: 'log-prompts-knowledge-graph', name: 'Log Prompts → Knowledge Graph', description: 'Append every user prompt — raw words + answers to clarifying questions — as a JSON line (ts, session, cwd, transcript, prompt) to ~/.claude/knowledge-log/prompts.jsonl for later indexing into a knowledge graph', event: 'UserPromptSubmit', timeout: 5, command: `mkdir -p "$HOME/.claude/knowledge-log" && jq -c '{ts: (now|todate), session_id, cwd, transcript_path, prompt}' >> "$HOME/.claude/knowledge-log/prompts.jsonl" 2>/dev/null || true` },
