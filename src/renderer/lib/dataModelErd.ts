...
export const ERD_ENTITIES: ErdEntity[] = [
  ...
  },
  {
    ...
    group: 'epic',
    store: {
      path: '<cwd>/session-manager-operations/prompt-sessions/exchanges.jsonl',
      format: 'jsonl',
      writer: 'epics',
      definedIn: 'src/main/exchanges.cjs ',
    },
    summary: 'One durable NDJSON record per completed terminal-chat exchange, appended off the hot path.',
    fields: [
      { name: 'ts', type: 'string' },
      { name: 'sessionId', type: 'string', key: 'fk', ref: 'epic.claudeSessionId', note: "the tab's claude session id" },
      { name: 'cwd', type: 'string', key: 'fk', ref: 'project.cwd' },
      { name: 'prompt', type: 'string' },
      { name: 'result', type: 'string' },
      { name: 'summary', type: 'string', note: 'produced by the shared Haiku summarizer' },
      { name: 'model', type: 'string' },
      { name: 'degraded', type: 'string', optional: true, note: 'set when summarization failed — the exchange itself is never dropped' },
      { name: 'promptId', type: 'string', optional: true, key: 'fk', ref: 'prd.slug', note: 'originating PromptTicket.id when dispatched from the per-tab queue; forward-only, never backfilled' },
    ],
    notes: [
      'Written via raw fs O_APPEND (not config.cjs writeJson) for atomicity across concurrent writers — assertOpsWrite is called explicitly since raw fs bypasses the normal enforcement point.',
      'Historical rows from before this store existed remain, untouched and unmigrated, in the old global ~/.claude/knowledge-log/exchanges/<encodeCwd(cwd)>.jsonl.',
    ],
  },
  {
    id: 'prd',
  ...
  },
  {
    ...
    group: 'memory',
    store: {
      path: '~/.claude/session-manager/agent-memory/<agentId>.json',
      format: 'json',
      definedIn: 'src/main/agentMemory.cjs (recordPath) ',
    },
    summary: 'Per-subagent memory, keyed by agent name rather than workspace cwd — one flat JSON file per agent, not a directory.',
    fields: [
      { name: 'id', type: 'string', key: 'pk' },
      { name: 'body', type: 'string' },
      ...
      { name: 'bytes', type: 'number' },
      { name: 'agentId', type: 'string', key: 'fk', ref: 'agentPersona.name', note: 'carried on AgentMemoryListResult, denormalized here — the file this entry lives in' },
    ],
  },
  {
    id: 'claudeTranscript',
  ...
  },
  {
    ...
    group: 'brief',
    store: {
      path: '<cwd>/session-manager-operations/project-pages/ (home.html)',
      format: 'directory',
      writer: 'project-home',
      definedIn: 'project_home_write MCP tool (src/main/lib/projectHomeAdminRoutes.cjs)',
    },
    summary: 'One self-contained overview page, written only via project_home_write.',
    fields: [
      { name: 'html', type: 'string | null' },
      { name: 'mtimeMs', type: 'number | null' },
    ],
  },
  {
  ...
  },
  {
    ...
    store: {
      ...
      format: 'json',
      definedIn: 'src/renderer/components/tabs/McpServers.tsx ',
    },
    summary: 'User-scope MCP server config — no single writer, since both the claude CLI and this app read/write the same file.',
    fields: [
      { name: 'name', type: 'string', key: 'pk' },
      { name: 'type', type: "'stdio' | 'http' | 'sse' | 'ws' | 'streamable-http'", optional: true },
      { name: 'command', type: 'string', optional: true },
      { name: 'args', type: 'string[]', optional: true },
      { name: 'url', type: 'string', optional: true },
      { name: 'env', type: 'Record<string, string>', optional: true },
      { name: 'headers', type: 'Record<string, string>', optional: true },
      { name: 'alwaysLoad', type: 'boolean', optional: true },
      { name: 'enabled', type: 'boolean', optional: true },
      { name: 'disabledTools', type: 'string[]', optional: true },
    ],
  },
  {
    id: 'pluginRegistry',
    name: 'Installed plugins manifest',
    aka: 'installed_plugins.json',
    group: 'config',
    store: {
      path: '~/.claude/plugins/installed_plugins.json',
      format: 'json',
      definedIn: 'src/renderer/components/tabs/Plugins.tsx ',
    },
    summary: 'Registry of installed plugin versions, keyed by "<name>@<marketplace>" — written by `claude plugin install`, read-only in the Plugins screen.',
    fields: [
      { name: 'pluginKey', type: 'string', key: 'pk', note: '<name>@<marketplace>' },
      { name: 'scope', type: 'string' },
      { name: 'installPath', type: 'string' },
      { name: 'version', type: 'string' },
      { name: 'installedAt', type: 'string' },
      { name: 'lastUpdated', type: 'string' },
      { name: 'gitCommitSha', type: 'string', optional: true },
    ],
  },
  {
    id: 'systemPrompt',
    name: 'User CLAUDE.md',
    aka: '~/.claude/CLAUDE.md',
    group: 'config',
    store: {
      path: '~/.claude/CLAUDE.md',
      format: 'markdown',
      definedIn: 'src/renderer/components/tabs/SystemPrompt.tsx ',
    },
    summary: 'The real ~/.claude/CLAUDE.md the claude CLI loads into every session it starts on this machine, in every project — edited here as plain markdown.',
    fields: [
      { name: 'content', type: 'string' },
      { name: 'imports', type: 'string[]', note: '@-imported files, listed in a left rail' },
    ],
  },
  {
    id: 'skillsAndCommands',
    name: 'Skills & Commands',
    group: 'config',
    store: {
      path: '~/.claude/skills/**/SKILL.md + ~/.claude/commands/*.md (and the <cwd>/.claude equivalents for project scope)',
      format: 'directory',
      definedIn: 'src/renderer/components/tabs/Skills.tsx ',
    },
    summary: 'User- and project-scope skill/command markdown files, each editable in place.',
    fields: [
      { name: 'kind', type: "'skills' | 'commands'" },
      { name: 'scope', type: "'user' | 'project'" },
      { name: 'name', type: 'string', key: 'pk' },
      { name: 'path', type: 'string', note: 'absolute path to the editable markdown file' },
      { name: 'dir', type: 'string | null', note: 'skill directory, if applicable' },
      { name: 'disabled', type: 'boolean', note: "skills only — disable-model-invocation: true frontmatter" },
  ...
  },
  {
    ...
    group: 'config',
    store: {
      path: 'in-memory result of `claude mcp list`, never persisted',
      format: 'in-memory',
      definedIn: 'src/main/mcpStatus.cjs (runProbe) ',
    },
    summary: 'Read-only, non-polling live connection probe — models an in-memory-only shape honestly, the same way transcriptEvent does.',
    fields: [
      { name: 'ok', type: 'boolean' },
      { name: 'servers', type: 'McpServerStatus[]', note: '{ name, target, transport, status }' },
...
export const ERD_RELATIONS: ErdRelation[] = [
  ...
  { from: 'exchange', to: 'epic', cardinality: 'N-1', label: 'session for', optional: true },
  { from: 'exchange', to: 'project', cardinality: 'N-1', label: 'belongs to' },
