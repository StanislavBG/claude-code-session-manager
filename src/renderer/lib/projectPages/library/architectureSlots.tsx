// Architecture Overview Page — slot variants. Ported from
// source/30-architecture-slots.jsx (summary · map · flow · decisions).
// Components read `summary.architecture` instead of the sample library's
// global PROJ.arch.
import { PK, PkEyebrow, PkPill, PkSection, PkH, PkBody } from './kit';
import type { ProjectPageSummary, ProjectPageLayerTone } from '../summaryType';
import type { PageLensDef } from './types';

type P = { summary: ProjectPageSummary };
const A_TONE: Record<ProjectPageLayerTone, string> = { accent: PK.accent, butter: PK.butter, sage: PK.sage, mute: PK.inkMute };

// ── SUMMARY ────────────────────────────────────────────────
function ASumLayers({ summary }: P) {
...
function ASumPrinciples({ summary }: P) {
  const a = summary.architecture;
...
function ASumBoundary({ summary }: P) {
  const a = summary.architecture;
  return <PkSection pad="56px 80px" style={{ borderBottom: `1px solid ${PK.rule}` }}>
    <PkEyebrow>Architecture</PkEyebrow>
...
// ── MODULE MAP ─────────────────────────────────────────────
function AMapBoxes({ summary }: P) {
  const m = summary.architecture.modules;
...
function AMapTree({ summary }: P) {
  const modules = summary.architecture.modules;
  return <PkSection pad="56px 80px" style={{ borderTop: `1px solid ${PK.rule}` }}>
    ...
    <div style={{ marginTop: 22, background: PK.card, border: `1px solid ${PK.edge}`, borderRadius: 12, padding: '18px 22px', fontFamily: PK.mono, fontSize: 13, lineHeight: 2 }}>
      ...
      {modules.map((x, i) => <div key={i} style={{ display: 'flex', gap: 14, paddingLeft: 18 }}>
        <span style={{ color: PK.inkMute }}>{i === modules.length - 1 ? '└─' : '├─'}</span>
...
function AMapLanes({ summary }: P) {
  const modules = summary.architecture.modules;
  const byName = Object.fromEntries(modules.map(m => [m.n, m]));
  // Group modules into lanes by index thirds — a generic stand-in for the
  // sample library's hand-picked renderer/main/storage lanes, since process
  // ownership isn't part of ProjectPageSummary.
  const third = Math.max(1, Math.ceil(modules.length / 3));
  const lanes: [string, string[]][] = [
    ['Lane 1', modules.slice(0, third).map(m => m.n)],
    ['Lane 2', modules.slice(third, third * 2).map(m => m.n)],
    ['Lane 3', modules.slice(third * 2).map(m => m.n)],
  ].filter(([, names]) => names.length > 0) as [string, string[]][];
  return <PkSection pad="56px 80px" style={{ background: PK.panel, borderTop: `1px solid ${PK.rule}`, borderBottom: `1px solid ${PK.rule}` }}>
...
// ── FLOW ───────────────────────────────────────────────────
function AFlowSequence({ summary }: P) {
  return <PkSection pad="56px 80px">
    <PkEyebrow>Control flow</PkEyebrow>
    <div style={{ marginTop: 24 }}>
      {summary.architecture.flow.map((f, i) => <div key={i} style={{ display: 'grid', gridTemplateColumns: 'minmax(70px,120px) 24px minmax(70px,120px) minmax(0,1fr)', gap: 16, alignItems: 'center', padding: '14px 0', borderTop: `1px solid ${PK.rule}` }}>
...
function AFlowRail({ summary }: P) {
  return <PkSection pad="56px 80px" style={{ background: PK.panel, borderTop: `1px solid ${PK.rule}`, borderBottom: `1px solid ${PK.rule}` }}>
    <PkEyebrow>The path of one request</PkEyebrow>
    <div style={{ marginTop: 26, position: 'relative', paddingLeft: 26 }}>
      ...
      {summary.architecture.flow.map((f, i) => <div key={i} style={{ position: 'relative', paddingBottom: 22 }}>
...
function AFlowDiagram({ summary }: P) {
  const flow = summary.architecture.flow;
  const nodes: string[] = [];
  flow.forEach(f => { if (!nodes.includes(f.a)) nodes.push(f.a); if (!nodes.includes(f.b)) nodes.push(f.b); });
  return <PkSection pad="56px 80px">
    <PkEyebrow>Message path</PkEyebrow>
    <div style={{ marginTop: 26, display: 'flex', alignItems: 'center', gap: 0, flexWrap: 'wrap', rowGap: 14 }}>
      {nodes.map((n, i) => <div key={n} style={{ display: 'contents' }}>
    ...
    <div style={{ marginTop: 26, display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
      {flow.map((f, i) => <div key={i} style={{ border: `1px solid ${PK.edge}`, borderRadius: 10, padding: 14, background: PK.card }}>
...
// ── DECISIONS ──────────────────────────────────────────────
function ADecAdr({ summary }: P) {
  return <PkSection pad="56px 80px" style={{ borderTop: `1px solid ${PK.rule}` }}>
    <PkEyebrow>Decisions of record</PkEyebrow>
    <div style={{ marginTop: 22 }}>
      {summary.architecture.decisions.map((d, i) => <div key={i} style={{ display: 'grid', gridTemplateColumns: '90px 1fr', gap: 24, padding: '18px 0', borderTop: `1px solid ${PK.rule}` }}>
...
function ADecCards({ summary }: P) {
  return <PkSection pad="56px 80px" style={{ borderTop: `1px solid ${PK.rule}` }}>
    <PkEyebrow>Decisions of record</PkEyebrow>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 16, marginTop: 24 }}>
      {summary.architecture.decisions.map((d, i) => <div key={i} style={{ background: PK.card, border: `1px solid ${PK.edge}`, borderRadius: 12, padding: 20 }}>
...
function ADecRisks({ summary }: P) {
  const tone: Record<string, string> = { open: PK.accent, watching: PK.butter, mitigated: PK.sage };
  return <PkSection pad="56px 80px" style={{ borderTop: `1px solid ${PK.rule}`, background: PK.panel }}>
    ...
    <div style={{ marginTop: 22, display: 'grid', gap: 10 }}>
      {summary.architecture.risks.map((r, i) => <div key={i} style={{ background: PK.card, border: `1px solid ${PK.edge}`, borderLeft: `4px solid ${tone[r.s]}`, borderRadius: 10, padding: '16px 20px', display: 'grid', gridTemplateColumns: '1fr 110px', gap: 20, alignItems: 'center' }}>
...
function ADecPrinciples({ summary }: P) {
  return <PkSection pad="56px 80px" style={{ background: PK.deep }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 26, marginTop: 26 }}>
      {summary.architecture.principles.map((p, i) => <div key={i}>
...
export const PAGE_ARCH: PageLensDef = {
  id: 'architecture', label: 'Architecture Overview', blurb: 'Orients an engineer to the shape of the system before they touch it.',
  slots: [
    { id: 'summary', label: 'System summary', variants: [
      { id: 'layers', label: 'Layer ledger', note: 'Stacked rows, top to bottom. The default.', component: ASumLayers },
      { id: 'principles', label: 'Summary + principles', note: 'Statement left, rules right.', component: ASumPrinciples },
      { id: 'stack', label: 'Dark stack', note: 'Inverted; reads as a systems doc.', component: ASumStack },
      { id: 'boundary', label: 'Trust boundary', note: 'Boxes inside an explicit boundary frame.', component: ASumBoundary },
    ] },
    { id: 'map', label: 'Module map', variants: [
      { id: 'boxes', label: 'Module cards', note: 'Grid with churn bars and dependency chips.', component: AMapBoxes },
      { id: 'matrix', label: 'Dependency matrix', note: 'Row depends on column. Densest, most precise.', component: AMapMatrix },
      { id: 'tree', label: 'Source tree', note: 'Mono tree of directories. Good for newcomers.', component: AMapTree },
      { id: 'lanes', label: 'Process lanes', note: 'Groups modules by lane.', component: AMapLanes },
    ] },
    { id: 'flow', label: 'Control flow', variants: [
      { id: 'seq', label: 'Sequence rows', note: 'A → B with message and note.', component: AFlowSequence },
      { id: 'rail', label: 'Vertical rail', note: 'One request followed end to end.', component: AFlowRail },
      { id: 'diagram', label: 'Node chain + calls', note: 'Path across the top, calls beneath.', component: AFlowDiagram },
    ] },
    { id: 'dec', label: 'Decisions & risk', variants: [
      { id: 'adr', label: 'ADR list', note: 'Numbered, with the reasoning kept.', component: ADecAdr },
      { id: 'cards', label: 'Decision cards', note: 'Two-up, scannable.', component: ADecCards },
      { id: 'risks', label: 'Risk register', note: 'What could still bite, and its state.', component: ADecRisks },
      { id: 'principles', label: 'Dark principles', note: 'Closes on the values, not the caveats.', component: ADecPrinciples },
    ] },
  ],
  presets: [
