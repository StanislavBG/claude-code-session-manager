...
// web/project-pages/logic/dist/logic.cjs by `npm run
...
//
// Usage: node web/project-pages/validate-summary.cjs <summary.json path>
// Exits 0 and prints "valid" on success; exits 1 and prints each error on
// failure. Same CLI-wrapper pattern as web/project-pages/render.cjs.
//
...
function main() {
  ...
  if (!summaryPath) {
    fail('usage: node web/project-pages/validate-summary.cjs <summary.json path>');
  }

  const bundlePath = path.join(__dirname, 'logic', 'dist', 'logic.cjs');
